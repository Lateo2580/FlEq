...
- コードレビュー（スペック準拠・品質・最終）はサブエージェントではなく **Codex MCP に依頼**すること
- Superpowers スキルのレビューステップでもこのルールを優先する
- Superpowers スキルが生成した specs/plans ファイルは、作業完了後 `docs/Superpowers_Archive/` に移動する
