...
- **実装委譲文には「最小実装・最小テスト」を定型で入れる**: 完了条件の列挙に続けて「仕組みを足すときは無いと何が壊れるかを 1 行で示す／テストは受入条件・契約境界・実不具合・corpus 履歴に限り、追加した各テストがどれに対応するかを成果物に 1 行で書く」を書く。gpt-6-astra への委譲では省略しない（2026-09-11 作者指示、詳細は `AGENTS.md` §コーディング規約）
- **ブラウザ capture の実走は親（Claude）が担う**: 子の sandbox は listen 不可。子は records に対する `--assert-from` で assertion を検証する
...
- セッション内で区切りがついたら（バグ修正完了、機能実装完了、調査結論など）作業ノート（repo 外）にセッションログを記録する
...
真実源は作業ノート（repo 外）のやりたいことリスト。**セッションを跨ぐ常設バックログ**で、実機観測・思いつき・持ち越しの置き場。
...
- **⏳待ち節**: 実事象（実警報の発表・実地震）や作者の裁定・外部レビューを待つだけの項目はここへ移す。**未着手ではなく待機**——Claude からは着手しない。条件が揃ったら元の節へ戻すか完了へ送る
- **裁定ラベル**: 自律サイクル（運用ルールは personal ブランチの `.claude/rules/autonomous-cycle.md`。main の作業ツリーには無いので `git show personal:.claude/rules/autonomous-cycle.md` で読む）で配送まで完走してよい項目には `🌙自走OK` と 6 要素ラベルを添える。**空欄が 1 つでもあれば配送不可**
- **セッション終了時（wrap-up）**: その回で完了・待機化した項目を反映してから閉じる。作業ノート（repo 外）のセッションログとバックログはセットで更新する

## 構造的欠陥台帳

真実源は作業ノート（repo 外）の構造的欠陥台帳（2026-09-09 作者提案）。バックログが「やること」、台帳が「局所修正で消えない構造の事実」。全面再構成の判断材料として育てる。
