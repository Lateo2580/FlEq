...
**特記（火山の state 保持単位）**: VFVO50/VFVO51/VFSVii の警報系列は `volcano:alert:${volcanoCode}`、VFVO52/VFVO56 の噴火系列は `volcano:eruption:${volcanoCode}`、VFVO54/VFVO55 の降灰系列は共通の `volcano:ashfall:${volcanoCode}` を `TelegramRevisionGate` の subject とする。各 family と active composite は 128 件を上限とし、`VolcanoStateHolder` が火山コードごとの alert／eruption／ashfall 三 slice、累積 source EventID、generation 1 state を一体で所有する。VFVO51 は数値レベルと非数値区分の双方を火山別 state entry に展開し、同一 subject が複数ある場合は電文順の最後を一度だけ適用する。state entry が空、または火山コードを確定できない入力は `volcanoStateMutationAccepted=false` の fail-open 表示に留め、holder・standby・通知・永続化を変更しない。非活性 entry は同じ火山の alert subject だけを `clearCurrent` する。tombstone は従来の standby guard と同じ 30 日/2 日で、gate の退場後は holder と standby も同じ active subject 集合へ同期する。現行の EventID 欠落噴火は live provenance として保持し、EventID 不一致時の一意 fallback は `legacyV1Fallback` を持つ旧v1 seedだけに限定する。空コード取消の再送は gate の `legacyRevisionKey` から tombstone subject を逆引きし、transient表示へ落とさない。rollback key は v2 に `eventId` / `codeFallback` provenance を保存し、空コード取消の EventID 逆引きには実 EventID 由来だけを使う。provenance 欠落の旧 v2 は誤取消を避けて逆引き対象外とする。EventID欠落取消は既存subjectの旧EventID rollback keyを維持する。v2 foundation の trusted gate は rollback 用 v1 `seen` の同一 key に無条件で優先し、旧 key (`volcano:alert:<code>` / `volcano:event:<EventID|code>`) へ dual-write する。`legacyRevisionKey` 導入前の active 噴火は canonical holder state の一意な火山コードから EventID を回収する。取消済みで EventID を復元不能な場合だけ code fallback を維持する。期限切れの旧 v1 噴火表示は取消対象 identity へ seed しない。VFVO53 の batch aggregator は durable な三 slice state とは別責務で、非 durable gate 通過後だけ集約する。v2 foundation を真実源とし、起動時 REST replay は全 owner の scratch transaction と 4 MiB full-file admission を通した同一 payload の再構成、または新しい受理報の適用だけに使う。
...
**Durable admission／repair contract**: standby に届く全 domain mutation は、gate・domain holder・standby projection・volcano repair state を clone した共通 transaction で組み立て、全 owner version token、domain count、volcano subtree 1 MiB、v2／v1 full-file 4 MiB、display wire preflightを mutation 前に満たす。commit 後に durable callback、その後に notifier／CLI／ticker／display side effectを行う。起動時の volcano repair は WebSocket acknowledgement generation と接続内 journal transport identityを固定し、REST cursor paginationで lower-bound coverageを証明してから最新 composition tokenへ rebaseする。coverage不能、generation不一致、journal gap、4 MiB超過は fail-closed で runtime を変更しない。
...
ProcessOutcome → toPresentationEvent() → PresentationDiffStore.apply()
  → summaryTracker.record() → display 配信
  → renderSummaryLine() (compact) or displayFn()
```

通知は `runDisplayPipeline` の前に実行される。受理済みイベントは常に表示する。
