<script lang="ts">
  ...
  let { item }: { item: Extract<ActiveStandbyCardV1, { kind: "nankaiTrough" }> } = $props();
</script>

<div class:critical={item.severity === "critical"} class="nankai-badge">
  南海トラフ臨時情報（{item.data.label}）発表中{#if item.restored}<RestoredChip />{/if}
</div>
