...
- 最小実装: 仕組み（型・層・抽象・設定・helper）を 1 つ足す前に「無いと何が壊れるか」を 1 行で示す。実装が 1 つしかない interface、製品が 1 つの factory、変わらない値の設定化、将来のための拡張点・scaffolding は作らない。既存の helper・型・パターンがあれば再利用する
- 受信経路の計算量: 受信・選択・結果照合 1 回の処理（hot path）で、全件×全件の走査や state・intent 全体の直列化（`JSON.stringify` での byte 計量・変更検出を含む）をしない。容量は subject や要素ごとの加算で数える。上限のある稀な一括処理（大量失効など）だけ O(N²) を許容し、その上限が保持件数と復元境界の両方で本当に効くことを確かめて、上限と計測値を契約に書く。2026-09-23、条文ごとに正しい latch 掃除が受信ごとの O(n²) になり、容量付近の受信 1 回が約 25 倍に後退した
- 最小テスト: テストは受入条件・契約の境界・実際に起きた不具合の再発防止・corpus 履歴（fixture→期待状態）に限る。実装の内部構造を写すテスト、同じ分岐の言い換え、fixture の焼き直し、private 関数ごとの suite は作らない。1 つの振る舞いに 1 テスト。既存テストの期待値を変える必要があるときは、変更前に理由を報告する
...
- **コード実装**: `npm run build` と `npm test` の両方が成功していること。テストを実行せずに「完了」と報告しない。追加・変更した各テストについて「受入条件／契約境界／実不具合／corpus 履歴」のどれに対応するかを 1 行で書く（どれにも当たらないテストは削る）
