...
> **裁定（2026-09-06 朝、ご主人）**: §6 の問いは 1B・2A・3A・4B・5A。範囲は (a) ラベル⇔数値の隣接と最大地域の明示だけの局所修正（後の全面再設計を控えて最小に）、結論 block は案 I、府県等一覧は見出し「府県等内の地域最大」で各行短く、ピーク開始時刻は次回全面再設計までカードから外す、期間別推移は次以降。実装 spec は `2026-09-06-typhoon-probability-label-adjacency.md`。
