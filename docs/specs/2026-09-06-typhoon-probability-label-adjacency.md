...
5. `data-ladder-stage`、`data-measurement-geometry-stage`、compressed、side-left / side-right / center placement と順序、rotation keys と順序 / tick / active key / position、Typhoon variant、visible cards は baseline と after で完全一致する。`rotationOmittedCount`、visible card の omitted / failure countも完全一致し、現行 baseline 0なら0とする。subsequence、rotation 短縮、compact → full 昇格を許容しない（現行固定 plan: `display/scripts/capture-legacy-standby.mjs:2049-2084`、現行比較 policy: 同 `:2447-2471`）。差が出た場合は期待表や manifest を更新して通さず、本実装を停止して実測差を報告する。
   > **裁定（2026-09-12、作者）: 1920×1080 max の配置変化を承認**（`approvedSpecificationChange`）。対象は `legacy-standby-gate|1920x1080|0|0|gateScenario=max&maxPlan=fhdMax` のみ。
   > 実 Chrome 30 cell の高さ（after − before）: 1280×720 max / compressed −9px、960×620 compressed +11px、1920×1080 max +9px、VPTA 単独 full −9px（変化は6 cell）。unresolved / overflow は全 cell 0。
   > placementLeft: `[tsunami,quake,weatherWarningForecast,typhoon]` → `[tsunami,quake,typhoon,volcano]`。
   > placementRight: `[weather,flood,volcano,heat]` → `[weather,weatherWarningForecast,flood,heat]`。
   > visible cards はこの左右配置変更だけを反映し、カードごとの omitted / failure、rotation・variant・overflow 等の他の frozen 項目と他29 cell は完全一致を維持する。
