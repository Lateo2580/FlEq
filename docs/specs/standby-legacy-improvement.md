...
- **正本**: 寸法・配置・意味論の正本は**モック v21（`3092514`）以降の最新コミット**。本文の数値はそこから写す。`5af389d` 以前のモックは目視裁定の視覚基線としてのみ参照する。
...
- **主比較規則（規範）**: 割当候補の優劣は辞書式で決める。
  ...
  4. 共通: ③左右列高差の小ささ → ④中央 overflow の少なさ → ⑤中央移動枚数 → ⑥移動枚数 → ⑦決定性 tie-break（canonical 順の配置キー列の辞書順最小）。
