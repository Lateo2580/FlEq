...
- **正本**: モック v26（commit `8d14ddc`）。`5af389d` 以前のモックは目視裁定の視覚基線としてのみ参照する。**数値の正本は本文の固定表（§5 期待 stage 表・§11.1 期待値表）であり、モックはその測定証跡**——将来モックと表が食い違った場合は表が勝ち、表の改定はご主人裁定を伴う spec 改版としてのみ行う。
...
- **時計は viewport（ticker を含む画面全体）の縦横中央に絶対固定**。時刻 92px・日付 21px・日付上余白 16px を全解像度で固定し、秒は時刻に対する相対比率を維持する。1920×1080 / 1512×982 / 1280×720 / 960×620 で時刻＋秒を一行に収め、秒を欠けさせない（2026-08-22 最終目視・packet 第四号のご主人裁定。中央列幅による時計寸法の揺れをなくすため）。
