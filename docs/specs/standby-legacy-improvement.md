...
- **正本**: モック v25（commit `c9eec14`）。`5af389d` 以前のモックは目視裁定の視覚基線としてのみ参照する。**数値の正本は本文の固定表（§5 期待 stage 表・§11.1 期待値表）であり、モックはその測定証跡**——将来モックと表が食い違った場合は表が勝ち、表の改定はご主人裁定を伴う spec 改版としてのみ行う。
...
- **期待 stage 表（v16 実測・v18/v21〜v24 で同値確認・rotationTick=0。§冒頭の数値正本規則に従い実装後の観測値で書き換えない）**:
