...
版: v4（ChatGPT Pro 独立レビュー IR01〜IR15 反映、ご主人裁定 IR05=A・IR11=A） / 2026-09-12  
...
|---|---|---|---|---|
| B01 `contracts-revision` | `ReportRef`、取得起源と独立した `Operation`、区域参照、特殊値、結果型、純粋な revision 比較 | 純粋な値型のみ | O01/O02、§5.1。先頭ゼロ、同一 revision 訂正、不明値、運用区分の一致・不一致・欠落 | 保存 schema を定義するが I/O・current を持たない |
...
```text
自プロセスで取込時刻・入力連番・取得起源を確定
  → frame／REST body の byte・外形検査
  → 有界 mailbox
  → decode・展開・全文 XML parse
  → metadata・運用区分・特殊値・意味入力を確定
  → route／family／運用区分付き subject／保存単位を確定
  → gate
  → 対象単位の reduceUnit
...
取得起源 `live／recovery／replay` と運用区分は別の必須契約とする。

```ts
type Operation = "normal" | "training" | "test";
```

- `live` は現在の購読から得た電文。自プロセスの取込時刻を受信時刻とする。
- `recovery` は過去状態を充足する取得。取込時刻と元報の発表・対象時刻を分け、現在受信した新報として扱わない。
- `replay` は固定時計と manifest を持つ検証入力。意味更新・intent 生成・射影は製品と同じ経路を通す。外部配送の無効化、試験専用保存先、実 adapter 使用を試験ごとに明示する。性能・保存・personal 検収で対象負荷を無効化した結果を製品条件の合格に使わない。
- `replay` や試験 harness からの入力であることを、電文の `operation:"test"` と同一視しない。

運用区分の判定源は `head.test`、本文の `Control.Status`、envelope の運用区分 metadata とする。B03 は取得時の値と存在状態を保持し、B04 が本文との照合後に区分を確定する。本文を読まず envelope の区分だけで意味状態を更新しない。

| 判定源 | 解釈 |
|---|---|
| `head.test=false` | `normal` と整合 |
| `head.test=true` | `training／test` と整合するが、二者を単独では識別できない |
| `Control.Status`、envelope の Status | 定義済みの通常・訓練・試験の値を、それぞれ `normal／training／test` へ正規化する |
| 形式上存在しない情報源 | `notProvided`。通常扱いの根拠にはしない |
| 当該入力形式で必須の情報源の欠落、不正値 | `rejected` |
| 存在する情報源どうしの矛盾、区分を一意に確定できない入力 | `rejected` |

対応する入力形式ごとに、必須 field・許可値・形式上存在しない field を B03/B04 契約で固定する。生 XML fixture に存在しない `head.test` を捏造しない。一方、必須 `Control.Status` の欠落を「情報源なし」として通さない。

拒否 reason は少なくとも `operationMissing／operationInvalid／operationMismatch／operationAmbiguous` を区別する。診断には判定源ごとの値・存在状態を有界に残す。矛盾時に true 優先・本文優先・通常への fallback で受理しない。

確定した区分は、出典 `ReportRef`、意味入力、subject identity、current、gate／watermark、取消記憶、batch、相関、通知 intent、公開 outcome、full／summary view、checkpoint と復元候補へ伝播する。通常・訓練・試験は同じ保存単位内の別 subject 空間とし、相互の取消・重複抑制・置換・coverage 充足を禁止する。保存単位・保存ファイルを区分ごとに増やさず、既存の件数・byte 上限は三区分の合計に適用する。

既存 checkpoint・移行入力で区分を確認できない場合、通常へ補完しない。根拠付きで変換できるものだけ移行し、判定不能部分は移行未確認として報告する。復元でも元の運用区分・通知期限を維持する。

`--test` による受信採否と訓練音の条件は Q7 の裁定対象とする。開始仕様案は **training／test の音を鳴らさない**。採用される通知・表示では区分を必ず明示し、実災害の警報と識別できない表現にしない。Q7 未裁定を通常扱いの根拠にせず、採否に依存する実装契約は裁定待ちとする。

`recovery` は同じ parser・gate・reducer を用いるが、途中結果を §10.5 の候補状態へ適用する。履歴報から現在向けの音・desktop intent・受信演出を作らず、中間 current を公開しない。採用は runtime の型付き入力で行う。

保存待ちは電文受理の完了条件にしない。直列化するのは状態遷移であり、I/O 中も次入力を扱う。I/O callback が current を直接変更することは禁止する。

`unchanged／rejected` では業務状態の参照と意味 revision を変えない。診断更新は別に許す。通知だけ・保存対象だけの変更も、その変更に従って結果型と世代へ反映する。変更しない枝は参照共有し、変更判定の全状態 clone・stringify と、受理 stack での外部 consumer 呼出しを禁止する。

P1 で三判定源の組合せと欠落・不正・矛盾を検証する。P2 で同じ EventID の通常／訓練 EEW の交差更新・取消・再起動を、P3/P4 で訓練津波の full／summary・復元 intent・表示区分を検証する。manifest の receive step は期待区分または期待拒否 reason を必須にする。
...
| rename後・ack前 | `uncertain`。当該単位の次の上書きを止め、2slot の世代・hash を再照合する。停止未確認の filesystem 操作が残る間は全体 writer を占有する。操作終了後の照合失敗は §5.8 の当該単位の再試行待ちとし、正常な他単位の保存を妨げない |
...
Q1 の「最後の未保存更新は最長 3 秒ぶん」は、保存開始までの期限ではない。**正常な保存環境では、各 dirty 更新の発生から 3 秒以内に、その更新を含む世代の保存完了を確認する**。

disk full・I/O 停止・成否不明中にも成立する電源断保証とはしない。未達時は Q2=B に従い最新表示を続け、影響単位と 3 秒を超える未保存区間を明示する。

全体 writer の in-flight は 1 件とし、単位ごとに最新 dirty 参照、`dirtySince`、`retryAfter`、連続失敗回数を持つ。失敗単位が即時再選択され、正常単位を永久に待たせることを防ぐための最小状態であり、別 queue や journal は作らない。

1. 新しく dirty になった単位は直ちに試行可能とする。後続更新で `dirtySince`・保存期限・失敗待ちの `retryAfter` を延長・初期化しない。
2. writer が空けば、**dirty かつ現在時刻が `retryAfter` 以上の単位のうち、最古の `dirtySince`** を選ぶ。同時刻なら固定 UnitId 順とする。intent の存在だけで他単位を無制限に追い越さない。
3. 選択時点の最新世代を capture して encode する。予約時の古い state を後で保存しない。
4. `reservedAt／capturedAt／writeStartedAt／ackAt`、対象世代、試行 ID を記録する。更新を包含する世代の ack までを 3 秒判定に用いる。
5. 終了が確認できた失敗は、失敗確定から **1、2、4、8 秒、その後最大 10 秒**を空けて当該単位だけ再試行する。encode 例外・単位固有の書込み失敗・再照合失敗を含む。`dirtySince` は失敗時刻へ置き換えない。
6. 確認済み成功で当該単位の失敗回数と再試行待ちを解除する。古い世代の ack は保存進捗だけを更新し、新しい current・取消・intent を巻き戻さない。
7. 試行可能単位がなければ最も近い `retryAfter` まで待つ。busy loop を作らず、新しい正常単位の dirty 発生では選択を再評価する。
8. 未保存年齢 3 秒超で `checkpointOverdue` を示す。接続正常を保存正常の代用にせず、失敗単位と正常単位の期限超過を別々に計数する。

`retryAfter` は実行時の再試行制御であり、業務状態・出典時刻ではない。再起動時は復元した状態から保存要否を再評価し、前プロセスの単調時刻を再利用しない。

一試行の監視期限は暫定 10 秒とする。停止を確認できない filesystem 操作は、timeout だけで writer から取り除かない。新しい書込みを重ねず、全体保存障害を表示する。遅延 ack は世代・試行 ID を照合する。`uncertain` の当該単位は照合完了前に上書きしないが、古い操作の終了後は、当該単位の再試行待ちを正常単位へ波及させない。

保存開始を 3 秒まで遅らせる debounce は置かない。合流は writer 待ちの最新参照への置換で行う。通常最大負荷で 3 秒条件を満たせなければ encode 量・I/O を減らして再測定する。単位追加分割や journal を自動追加せず、正常系の期限変更は Q1 の変更として扱う。

E14 に「一単位だけ永続失敗＋他単位正常」を含める。終了済み失敗の試行間隔、正常単位の 3 秒以内の保存完了、停止未確認時の書込み重複 0 を別々に判定する。凍結担当は checkpoint 担当と統合担当、時期は P2 の B07 契約前とする。
...
interface NotificationIntent {
  id: string;
  unit: UnitId;
  subject: string; operation: Operation;
  source: ReportRef;
...
- `id` は保存単位、運用区分、subject、電文識別、意味 revision、遷移、channel から決定する。区分を `payload` の任意表示 field だけへ埋めず、復元・選択・取消時にも必須値として照合する。
...
- desktop と sound の成否を別々に記録する。`channel:"sound"` の音源は、保存単位の分野（気象／火山／地震・EEW／津波）× 段階で選ぶ。分野別根音表（`docs/specs/sound-design-system.md` §分野別根音表 v1、2026-09-17 統合済み）は新築で実装し、旧築の 5 段階 `playSound(level)` には入れない（2026-09-15 ご主人裁定 B）。Linux の再生列は旧築と同じ `ffplay → paplay → aplay` を使い、起動時 probe も同じ列を辿る（Issue #20）。

### 6.3 試行・選択順・保存順序

1. reducer が意味変更と区分付き intent を同時に返し、runtime が同じ保存単位へ採用する。
2. 保存を予約するが、通知は保存完了を待たず試行できる。緊急通知を fsync 待ちにしない。
3. 空いた channel は、有効かつ `nextAttemptAt` 到達済みの intent から選ぶ。
4. 固定優先群は **通常運用の EEW → 通常運用の津波緊急 → その他**。津波緊急は大津波警報・津波警報の発令・引上げとする。区分や優先度を自由な数値設定にしない。同群は期限、生成時刻、intent ID の順とする。
5. 試行ごとに `attemptId`、AbortSignal、単調時計の timeout を与える。
6. 結果を runtime 入力へ戻し、現在の intent・区分・attemptId・期限・置換状態と照合する。
7. 有効な成功だけを delivered にして同じ単位を dirty にする。失敗・timeout は元の期限内だけ再試行対象とする。

正常 adapter と固定した合法負荷における、緊急 intent の生成から**初回 adapter 呼出しまでの待ち時間は暫定 1 秒以内**を開始目標とする。保存待ち・queue 待ち・進行中処理の停止待ちを含め、選択可能になった時刻から測り直さない。音の終了・人の認知までの値ではない。

緊急 intent 到着時に下位群を実行中なら、その試行結果を無効化して abort を要求する。通常の channel timeout 満了まで待たず、既存の停止確認猶予内で処理を閉じる。外部へ既に出た音・通知を巻き戻したとは扱わない。上位群が下位群の再試行より先に選ばれること、同群・上位群の処理がある場合も初回待ちの実測から除外しないことを契約とする。

取消・置換・期限・試行 timeout でも結果を無効化し、abort を要求して論理 channel を解放する。遅い成功で delivered や別試行の結果を書き換えない。中断した intent がなお意味上有効なら、元の期限を維持して再試行規則へ戻す。

論理 channel の解放と外部実処理の停止は別だ。停止確認できない adapter は隔離・無効化し、同じ実処理へ次を重ねない。channel は新しい intent を有界に保持できるが、利用可能になるまで試行せず、期限到達で欠落を記録する。隔離中の欠落を 1 秒条件の成功として数えない。

Q7 で採用される training／test 通知は区分を明示し、通常運用の緊急通知を追い越さない。訓練音は §5.1 の開始案と Q7 に従う。

成功は adapter の送信完了であり、人が見聞きした保証ではない。保存前 crash の欠落、送信後・delivered 保存前 crash の重複を許容し、exactly-once を主張しない。
...
| その他 sound | 元の生成から 60 秒以内。津波緊急も TTL と初回待ちを別判定 |
| その他 desktop | 元の生成から 180 秒以内 |
| sound の一試行 timeout | 暫定 10 秒。緊急到着による下位群の中断は §6.3 |
| desktop の一試行 timeout | 暫定 5 秒。緊急到着による下位群の中断は §6.3 |
| 実際の試行期限 | channel timeout と intent 残存期限の短い方。取消・置換・優先中断は先に適用 |
| abort 後の停止確認猶予 | 暫定 1 秒。未確認なら adapter を隔離・無効化 |
| 緊急群の初回試行待ち | 生成から呼出しまで暫定 1 秒以内。停止・dispatch の費用も含む |
| 再試行間隔 | 失敗確定から 1、2、4、8 秒、その後最大 10 秒。元の期限を延長しない |
| pending intent | 保存単位ごと最大 128 件かつ 128 KiB。運用区分・channel を合算 |
| 同時試行 | channel ごと 1 件。停止未確認の実処理を別に計数し、重ね呼出ししない |
| 完了・失効記録 | family の重複判定に必要な期間だけ保持 |

timeout は adapter ごとの宣言定数であり、利用者設定を増やさない。sound 10 秒は平常時の音を途中で切りすぎない開始値、desktop 5 秒は応答不能を EEW intent の全寿命より短くする開始値、緊急初回待ち 1 秒は通常 backlog への埋没を防ぐ暫定目標だ。

停止確認猶予 1 秒を丸ごと消費すれば dispatch を含む初回待ち目標を超え得る。実 adapter の検収では合計時間で判定し、目標未達を timeout の宣言だけで合格にしない。B09 担当と統合担当が P2 前に EEW、P3 の津波契約前に津波緊急の測定条件と採用値を凍結する。

満杯時は期限切れ・superseded・完了済みを先に回収する。それでも不足する場合、当該単位の待機中 intent と新 intent の間で §6.3 の同じ順序を適用し、下位群を保持したまま収容可能な上位群を拒否しない。必要な件数・byte 分の下位候補を外し、欠落理由・件数を記録する。停止未確認の実処理を回収済みと偽らない。他保存単位を同期 commit するための追出しは行わない。

新報というだけで旧 intent を全削除しない。取消・解除・意味上の置換は意味モジュールが決め、区分をまたがない。recovery 履歴から新しい期限付き intent を作らず、復元後の再試行も元の `createdAt／expiresAt` を使う。

E21 で通常 backlog・再試行・満杯・通常試行中に津波緊急／EEW を投入し、選択順、初回待ち、実処理停止、欠落、遅い成功の無効化を検証する。
...
入力側の固定優先順は、必要な control・期限・保存 ack の進行を保ったうえで、**EEW 候補 → 津波警報候補 → その他**とする。通常 backlog の後ろへ緊急候補を固定しない。

- 外形 metadata で確認できる route を選択根拠にする。津波の階級判定だけのために main で XML を追加 parse しない。
- 津波候補は VTSE41 を保守的に対象とし、発令だけでなく解除・降格も通常 backlog を追い越せるようにする。本文確定後の意味処理・通知生成は各契約に従う。
- 未検証 metadata は優先処理の候補判定にだけ使い、運用区分・発令・取消の確定に使わない。確認済み training／test を通常運用の緊急入力より優先しない。
- 同一 `operation／family／subject` の順序を維持する。先行する同対象入力がある場合、その入力を飛ばして後続だけ採用しない。
- A では実行中の非中断処理を途中で追い越せない。その待ちを §7.5 の遅延に含める。
- 入力連番、受信順、実際の処理順、選択理由を比較記録に残す。検収時の選択規則を固定し、全 family に順序無関係性を要求しない。

§7.2 の緊急予約枠は EEW・津波候補・必要 control で共有し、枠を別々に増やさない。満杯時は §7.3 に従い、欠落を隠さない。

入力待ちには §9.9 E07 の上限を適用するが、緊急入力はそれに収まるだけでは合格にしない。受信から実表示までの E01、p99 250ms を優先する。津波通知の優先群は §6.3 であり、入力側で解除・降格を優先することと混同しない。

### 7.5 EEW・津波の受信から実表示までの測定プロトコル

本節を緊急表示の共通測定プロトコルとする。**IR05=A：大津波警報・津波警報も EEW と同じ p99 250ms を開始目標とする。** P2 の A/B 判定前に EEW 条件、津波を閉じる P3 契約前に津波条件を凍結し、P4/P5 では製品経路で再検収する。

測定担当が manifest を作り、統合担当が入力・時計・描画証拠・集計式を確認する。結果を見て母集団・除外条件を変更しない。

| 項目 | 固定条件 |
|---|---|
| 起点 T0 | main が完全な WS frame を受け取った callback の入口。decode 前の単調時刻 |
| 中間点 T1〜T3 | mailbox 入場、worker 処理開始、採用・公開用射影完了を対応付け、queue と処理の待ちを分離 |
| T4 | 対象 snapshot を SSE writer へ渡した時刻。送信完了とは呼ばない |
| T5 | Chrome が対象 snapshot を受け取った時刻 |
| EEW の T6 | 対象更新を反映したカードと必要な予想震度地図が初めて同時に実 paint された時刻 |
| 津波の T6 | 対象更新を反映した大カード・警報区分・必要な海岸線が初めて同時に実 paint された時刻。解除では旧表示の除去、降格では区分・色・点滅・文言の更新完了 |
| 対応 ID | run ID、input ID、operation、対象 subject、意味 revision、snapshot の streamId／sequence |
| ブラウザ | 開発機 Chrome の前景 tab。版・viewport・DPR・地図資材 hash・motion 設定を固定 |
| warm-up | 各 run の最初に測定対象と別 ID の固定入力 100 件 |
| 正式標本 | 各母集団 1,000 件を 3 run。各 run と合算を報告し、各 run が合格すること |
| 分位点 | 昇順 n 件の `ceil(p × n)` 番目。補間しない |
| 欠落 | T0 から 10 秒までに期待する実 paint がないもの。欠落 1 件でも不合格 |
| 報告 | p50／p95／p99／max、標本数、欠落数、時計不確かさ、trace 欠落数 |

EEW は次を別母集団で測る。

1. 固定した通常負荷・backlog 中。
2. 最大正常 XML の full parse 開始直後。
3. 最大正常 checkpoint encode 開始直後。
4. P4/P5 では容量縮退中、および津波同時表示中。P5 は旧築＋新築＋採用 personal の最終構成で 1〜4 を再測定する。

津波は **発令・引上げ・区域拡大**と**解除・降格**を別系列とし、各遷移を識別して報告する。各系列について、通常 backlog、大型 parse 直後、checkpoint encode 直後、EEW 同時、容量縮退を別母集団にする。大津波警報・津波警報と、そこから注意報・解除へ移る期待値を manifest に固定する。軽い発令だけの集計で解除・降格を代替しない。

parse／encode 直後は開始から 1ms 後を目標に投入し、実際の T0 が開始後 0〜5ms、かつ対象処理が進行中であることを正式条件とする。投入失敗は理由付きで別記する。処理が短く条件を作れない場合は所要時間と試行を報告し、統合担当が判定前に代替条件を固定する。

各試行は既知の初期状態から開始し、後続入力が対象をすぐ置換して描画不要にする条件を避ける。burst・相互取消の正しさは別の契約系列で検証する。

容量縮退系列は、他分野の縮退中と対象分野自身の fallback を分ける。通常合法最大 manifest は E13a を満たし、危険区域の省略を T6 の軽量化に使わない。対象分野の意図的な予算超過系列だけは §8.1 の最小事実・緊急配置・省略表示・旧地図除去を T6 の期待値とする。個別詳細の取得完了を 250ms の必須経路へ入れない。

**時計対応**：

- Node と Chrome の単調時計を開始時・終了時・実行中 30 秒ごとの往復測定で対応付ける。通信の非対称遅延をゼロと仮定しない。
- 各標本を誤差込みの遅延区間 `[L_i, U_i]` とし、`p99(U_i) ≤ 250ms` は合格、`p99(L_i) > 250ms` は不合格、それ以外は未確認。
- 時計対応の区間幅は暫定 5ms 以下。超過時は測定改善を先に行い、A 合格・B 必須の根拠にしない。

Chrome の描画 trace と画面記録を対応付ける。DOM 更新・rAF callback だけを実 paint に置き換えない。保存完了・通常表示 debounce・ページ巡回を緊急表示の前提にしない。

P2 は EEW の最小実描画経路、P3 は津波の最小実描画経路、P4/P5 は製品実装を使う。背景 tab は別の復帰試験とし、前景性能へ混ぜない。§7.6 の分岐は、同じ測定定義で原因を特定した未達を根拠にする。

### 7.6 A の未達原因と parse 分離 B

§7.5 の未達、緊急予約枠の反復枯渇、期限・緊急処理の停止が確認された場合、A のまま合格にしない。health の応答だけで worker の進行を証明しない。

まず T0〜T6 と七区間計測から、待ちの主因を XML parse、checkpoint encode、射影・整形、転送・描画へ帰属させる。複合原因は別々に記録する。
...
条件付き B 移行そのものは裁定済みであり、parse 起因の根拠が出た後に A/B を未決へ戻さない。異なる原因の解決案や目標変更を、この裁定に含めて承認済みとは扱わない。
...
| 意味対象の受理 | `operation／family／subject／影響範囲` ごとの最新候補出典、decision・reason、現在の受理出典・意味 revision | 公開 revision 比較で現在より新しい候補が `unchanged/stale` 等で不採用なら、その対象に `freshnessSuspect` を記録する。新旧不明は不明とする。解除は疑っている同じ対象・範囲を確認する正常採用、または coverage 確認だけで行う。別官署・別 subject・別区分の受理や heartbeat では解除しない。部分的新報で単位全体を確認済みにしない §5.6 の規則を通常稼働にも適用し、current を自動補修しない |
...
### 7.10 worker 内の詳細ページ射影

summary で省いた詳細の取得は、既存の B05 MessagePort を使って **worker が要求された一ページだけを射影する**。main は最新 snapshot 一枚を保持する方式を維持し、別の full view・旧版詳細・別 current を持たない。

B05 は要求の計数・取消・応答照合、B08 は現在状態からの射影、B11 は認証・HTTP 入出力を所有する。
...
type DetailRequest = Readonly<{
  ...
  domainId: UnitId;
  operation: Operation;
  requestedVersion: DisplayVersion;
...
type DetailResponse =
  | Readonly<{
      ...
      operation: Operation;
      requestedVersion: DisplayVersion;
      observedVersion: DisplayVersion;
      contentRevision: string;
      page: number;
      pageCount: number;
      payload: DomainDetailPage;
    }>
  | Readonly<{
      kind: "error";
      requestId: string;
      reason: "invalidRequest" | "versionChanged" | "busy"
        | "unavailable" | "timeout" | "cancelled";
    }>;
```

`DomainDetailPage` は B08 の分野別公開 DTO の union とし、許可 field・行／項目境界・最大数を P4 前に固定する。任意 object・関数・raw XML・内部 state を通さない。要求の文字列長、page の整数範囲、分野・運用区分を受付時に検査する。

| 対象 | 契約 |
|---|---|
| 要求総数 | 全 client 合計 8 件。受付済みから HTTP 応答の排出・終了までを含む |
| 要求 byte | UTF-8 JSON で 1 件 1 KiB、合計 8 KiB 以下 |
| worker 内の射影 | in-flight 1 件。通常受理より下位で開始し、control・期限・緊急入力を待たせない |
| 応答 byte | wrapper を含む UTF-8 JSON で 1 応答 64 KiB 以下 |
| main の応答保持 | 最大 8 応答・合計 512 KiB。全詳細 cache に転用しない |
| client | 同時取得 1 件、保持は現在開いている詳細の現在ページだけ |
| timeout | B11 の受付から応答本文の排出まで暫定 2 秒。queue・射影・転送待ちを含む |
| worker 停止 | 新規・待機要求に 503 相当。既存 snapshot を詳細応答に偽装しない |
| 満杯 | 429 相当。client 数・要求数を増やして逃げない |
| 版不一致 | 409 `versionChanged` |
| timeout | 504 相当。既に本文送信中なら通信を終了し、不完全 JSON を正常応答にしない |

要求された `streamId` と対象内容版を射影前に照合する。worker が実際に参照した公開版を `observedVersion` として返す。他分野・保存 metadata だけが更新された場合は要求 snapshot と異なる sequence でもよいが、対象内容版は一致しなければならない。対象が射影中に変更された場合も、異版のページを混ぜず 409 とする。

取消・client 切断・timeout では request ID を無効化し、待機中なら取り除く。開始済みの処理には取消を通知し、遅い応答を廃棄する。実処理終了の確認前に worker の in-flight を解放して次の射影を重ねない。worker 無応答は §7.8 の停止判定へ渡す。

詳細が通常入力の継続で開始できず timeout する場合は取得失敗を返す。詳細を成功させるために通常受理の優先度を下げない。一方、開始済み射影の非中断区間も E01 の負荷条件に含め、64 KiB 応答上限だけで CPU 時間を保証したとは扱わない。

停止未確認の処理、待機要求、排出待ち応答はそれぞれ有界に計数する。取消通知用の別無限 queue、汎用 broker、世代別詳細アーカイブは作らない。B05/B08/B11 担当と統合担当が P4 着手前、wire 型を先行使用する場合はその前に凍結する。
...
- `full` は安定した事象・カード ID、運用区分、情報種別、更新時刻、出典、復元状態、区域コードと体系、数値・時刻・件数・代表地名、有界な詳細、EEW 予想震度区域、津波予報区・警報区分、有効期限を含む。観測等の詳細を先行して個別取得へ分ける場合、その省略範囲と §7.10 の参照を明示する。危険区域の必須事実まで取得待ちにしない。
- `summary` は、現在有効な **運用区分・情報種別ごと**に情報種別・最上階級・対象区域数・時刻を含む。区分をまたいで最大階級や区域数を合算しない。区域数は同じ体系内の重複を除き、異なる体系の同一性を推測しない。時刻は要約を構成する意味更新の最新時刻とし、生成時刻で代用しない。未提供の階級・時刻は型付き不明値にする。training／test の識別表示を見出し・詳細・避難文言の表示時にも省略しない。
...
U-E／U-T の容量縮退中も緊急表示モード、EEW 宣言高、津波の左列、全体図固定、大津波警報の「今すぐ避難！」と運用区分の明示を維持する。未取得の区域別値・geometry は推測せず、省略中を示す。ただし **P4 前に固定した通常合法最大 manifest では、U-E の区域別予想震度と U-T の予報区別警報区分の配送欠落を 0 とする（E13a）**。観測詳細の先行分離または分野予算の配分が必要かを実測し、不要なら仕組みを足さない。F01=A の最終 fallback は維持するが、正常容量の不足を summary の合格条件で免除しない。意図的な予算超過系列は最小事実・緊急配置・省略表示・詳細到達を検証し、通常合法最大系列と混ぜない。
...
2. 要求には対象分野の `contentRevision` と運用区分も付ける。応答は要求版 `requestedVersion` と実際に参照した公開版 `observedVersion`、対象内容版、分野・区分・ページ番号・ページ数を返す。一応答へ異なる対象内容版を混ぜない。型と worker 経路は §7.10 に従う。
...
4. client は応答の request ID、対象分野、運用区分、streamId、対象内容版が現在の要求・表示と一致する場合だけ採用する。対象の取消・失効・内容変更・streamId 変更で無効化する。他分野だけの更新による sequence 差では捨てない。worker は要求時の旧版を保存・再現せず、その時点の現在状態だけから射影する。
...
7. client は現在開いている詳細の現在ページだけを保持し、in-flight は 1 件。main は full view を保持せず §7.10 の worker 射影を中継する。要求総数・byte・応答保持・優先順位・取消・2 秒 timeout・worker 停止応答も同節に従う。詳細経路は既存 display API と同じ認証境界に置き、snapshot 配送を取得完了待ちにしない。
...
4. summary にも運用区分別の全情報種別の最小事実が収まることを保証する。三区分を合わせて既存の分野予算内とし、訓練表示や障害表示を削って収めない。文字列長・項目数・診断表現を有界にする。P4 の容量契約では E13a を先に検収し、必要な観測詳細分離・分野予算変更を固定してから F01=A の fallback を適用する。
...
**裁定済み**：通常時・片側集約・EEW 大カードと津波大カードの同時表示のいずれでも、器に収まらない対象はページ送りにする。件数だけの要約へ畳んで対象を隠す方式は採用しない。ページごとのドットを置き、クリックでそのページへ切り替えられるようにする。
...
5. 内容更新では現在ページ先頭の安定 ID を含むページを維持し、消えた場合は旧ページ番号を範囲内へ clamp する。ただし自動巡回の期限到達時は巡回を進める。新着・取消・再分割で未表示の対象を表示済みにしたり、一巡の開始時刻・継続対象の未表示時間を初期化したりしない。未表示ページの進捗は、そのページが含む安定した対象 ID に対応付けて引き継ぐ。消えた ID だけを除き、新しい ID は未表示として加える。metadata 更新ではページも進捗も変えない。
...
ページ変更時は旧ページに属する通常の詳細 overlay と受信演出を終了する。手動停止中は新着による自動ページ移動を行わない。自動送り中は新着の対象ページへ移ってよいが、通常巡回の次回期限と未表示対象の進捗を巻き戻さない。受信演出の都合で期限を延ばさず、次の巡回に必要なら演出を短縮・省略する。EEW・津波大カードはページ送りと別に即反映し、モード変更後も手動停止と継続対象の進捗を引き継ぐ。

自動送りの一巡上限 `L` は **Q5-b の推奨 60 秒、P4 前裁定**のまま保留する。採用後は滞在時間の総和だけでなく、継続して有効な各対象 ID の初回表示待ちと実 paint 間隔の最大値が `L` 以下であることを判定する。未表示対象を通常巡回で先に処理し、受信演出で既表示ページへ戻っても未表示対象を後回しにし続けない。scene が持つ進捗は現在有効な ID の範囲に限定し、過去全件の履歴を作らない。新着・取消・再分割・単独／同時遷移を続ける系列で検収する。手動停止・手動詳細・背景 tab の時間は別報告とし、自動再開時から保証を再開する。自動受信演出による停止時間は除外しない。最小読取時間と `L` が両立しない最大件数・入力頻度は P4 前に Q5-b の裁定材料とし、黙って高速送りや期限延長をしない。
...
自動詳細を省略する場合は、宣言したフラッシュの終了で演出を終了する。個別取得の待機上限は §7.10 と共通の暫定 2 秒とし、失敗・版不一致・timeout では待機を終了し、その演出が所有する camera だけを復帰させる。手動詳細では取得失敗表示と再取得操作を残す。受信演出の 6 秒保持も §8.5.5 の巡回期限を延長せず、必要なら短縮する。
...
フラッシュは現在 camera 内で可能な範囲に行い、画面外の対象へ寄ることを手動状態維持の例外にしない。手動詳細を自動詳細で置き換えない。対象カードがページ外なら、手動停止中はページを動かさず演出を省略する。自動送り中の対象ページへの移動は §8.5.5 の巡回期限・未表示進捗を維持できる場合だけ行い、camera・県フォーカスを連動させない。新着のたびに先頭ページへ戻して timer を再開する実装を禁止する。
...
- EEW は右列に宣言高の大カードを表示する。
- 地図を主役に保ち、陸を予想震度で色分けする。
- 揺れの到達時間・猶予秒・countdown をカードに載せない。
- current・取消・期限は engine が所有し、演出終了を解除と同一視しない。

EEW の対象は、一件の新着ではなく**現在有効な EventID 集合**とする。identity は運用区分を含め、同じ EventID の通常・訓練・試験を混同しない。

津波表示がない場合、自動 camera の bounds は、その集合に属する予想最大震度 3 以上の区域の和集合から計算する。代表点ではなく全対象 geometry を用い、欠損区域を除外して「全部入った」としない。不明・範囲値は意味型の規則に従い、文字列比較や不明値の 0 扱いで対象から落とさない。

一件の取消・失効ではその事象だけを集合から外し、残存事象の大カード内容・地図・緊急モードを維持する。EEW 終了は集合が空になった時点とする。大カード内の一覧が宣言高を超える場合は既存の内部分割とドットで扱い、別の画面方式を追加しない。

EEW の camera 割込みは通常受信より優先する。津波なしでは退避した手動状態より EEW bounds を優先する開始案を維持する。EEW 中の新たな手動 camera 操作は保持し、古い復帰処理で戻さない。津波表示中は全体図固定を優先する。

#### 8.7.2 津波

以下は裁定済みとする。

1. **T1=A**：地図主役を維持し、左列に津波大カードを置く。
2. **T2**：海岸線は大津波警報だけ点滅、津波警報は赤・静止、津波注意報は黄・静止。大津波の紫 6px と正規予報区 GIS の島保持は P4 で検証する開始案。
3. **T3=A**：津波表示中は zoom なし・全体図固定。EEW 同時でも陸は予想震度、海岸線は津波を描く。
4. 大津波警報の直下の避難指示帯は **「今すぐ避難！」**。テロップも一致させる。training／test では同じ表示面に区分を明示し、通常の警報に見せない。
5. 津波単独なら他カードを右へ、EEW 単独なら左へ集約し、あふれはページ送りとドットで扱う。

津波大カードの有効性は、engine が公開する現在有効な津波表示対象から決める。大津波から津波警報・注意報への降格だけで大カードを消さない。取消・失効後も他の有効対象があれば残す。

大カードには予報区ごとの予想高さ・到達予想時刻・観測値・警報／注意報区分を載せる。予想と観測、到達予想と観測時刻を混同せず、未提供値を計算して埋めない。観測詳細の個別取得と危険区域の配送条件は §8.1・E13a に従う。一覧のあふれはカード内でページ分割する。

海岸線と hover／受信時の引き出し線は別物とし、引き出し線を常設しない。モックの県輪郭を正規津波予報区 GIS として採用せず、予報区コード・範囲・島・複数区間に対応する資材を用意する。現行 `display/maps/quake/source.lock.json:11`、同 `:44` の入力に津波予報区 GIS は含まれない。

#### 8.7.3 片側集約と同時発生

片側集約では反対列のカードをすべて 48px の見出しにし、同じページの空きに応じて重大度上位から通常カードへ戻す。直近の地震は集約中は見出しを維持し、通常カード化のためにページを増やさない。

見出しでも収まらなければ先に全対象をページ分割し、各ページの残余で昇格を計算する。取消で空きが増えれば同じ算術で自動昇格する。重大度・同順位・ページ進捗は §8.5 に従い、テロップは下段に残す。

**同時モードは、EEW 大カードと津波大カードが両方有効であることだけで定義する。津波の階級から切り離し、F08=A の配置を適用する。**

- 左列に津波大カード。
- 右列上部に EEW 大カード。高さ `H_EEW` は採用 profile の宣言値で固定。
- 時計・余白・gap・ページドットを予約した右列残余に、他カードを 48px 見出しだけで重大度順に配置。同時モードでは通常カードへ昇格させない。
- あふれはページ送りとし、全ページをドットで選択可能にする。
- 両大カードを他カードのページと一緒に消さない。
- 陸は予想震度、海岸線は津波、camera は全体図固定。

右列全高 `H_R`、時計等の予約 `R`、大カードとの gap `G`、ページ操作予約 `P` に対し、他カードの利用可能高は `H_R - R - H_EEW - G - P` とする。見出し間 gap も算入し、DOM 計測を使わない。対応 viewport では見出し 1 本とページ操作が必ず収まる profile を N17 で固定する。

EEW 継続中の大津波→警報→注意報では、階級に応じて色・点滅・避難文言を更新し、同時配置を維持する。津波対象がなくなれば EEW 単独へ、EEW 集合が空なら津波単独へ移る。注意報中に EEW が始まる場合も同じ規則を使う。

#### 8.7.4 緊急表示中の県フォーカスと復帰

EEW または津波大カードが有効な間は、県フォーカスによる **dim と県選択の強調を保留**する。危険区域が選択県外であることを理由に、色・不透明度・点滅を弱めない。camera を手動操作した場合も、緊急危険層へ県 dim を復活させない。

通常から最初の緊急表示へ入る際、手動 camera と `focusedPrefecture` を一組だけ退避する。単独→同時→単独で退避を重ねず、両方の緊急表示が終了するまで復帰しない。EEW 中の明示的な手動変更は復帰先へ反映し、古い退避値で上書きしない。津波中に禁止された camera 操作は受理しない。

全緊急表示の終了後に有効な退避状態を復元する。対象県・camera が不正なら全体図・県選択なしへ戻して理由を示す。通常受信演出の timeout はこの所有を解除できない。

運用区分をまたいで危険値を上書き・合算しない。Q7 で通常と訓練の同時表示を採る場合も、通常運用の危険区域・緊急表示を訓練側の取消・camera 要求で隠さない条件を P4 前に固定する。
...
県クリックによる拡大では `focusedPrefecture` を保持し、平常時は他県と他県の市区町村を dim し、対象県の縁を太く最前面へ出す。選択範囲外の click で解除し全体図へ戻す。EEW／津波の緊急表示中は §8.7.4 に従って dim・県選択強調を保留し、危険色を減光しない。
...
津波の全体図固定を守るため、津波表示中は camera を変える県選択・wheel・drag を停止し、hover・名札・カード詳細は使用可能とする開始案を採る。固定理由を表示する。手動 camera と県フォーカスの退避・復帰は §8.7.4 の一組に統一する。津波終了時に EEW が残れば EEW の規則へ移り、両方の緊急表示が終了するまで通常状態へ戻さない。
...
| EEW・大津波警報・津波警報の受信→実表示 | §7.5・E01 の p99 250ms。層切替は E25、操作フレームは E26 で別に測る |
...
| O04 | 8日前の VPWS50 current＋新しい正常全国報、全国報不採用→別官署 partial 受理→全国確認 | stale lock から脱出し有効 partial を維持。故障注入で全国報が不採用の間、別官署・別 subject・別運用区分の正常受理では全国の `freshnessSuspect` を解除しない。同じ対象・影響範囲の正常採用または coverage 確認でだけ解除 |
...
| O09 | 最大 XML／encode 開始→EEW・津波、通常 backlog、burst、EEW 同時、容量縮退、personal 有効 | §7.5・E01 の各母集団で実 paint p99 250ms。津波の発令・引上げ・区域拡大と解除・降格を分離。main health、queue 待ち、固定優先順、原因別 A/B 判定 |
...
| D-AC08 | ページ到達 | 通常・片側・同時・伸縮・テロップ・内部分割で、ページ集合の対象 ID が有効対象集合と一致。ドットで全ページへ到達し、件数だけの畳みによる到達不能 0。新着・取消・再分割でも継続対象の未表示進捗を巻き戻さない |
| D-AC09 | 手動ページ調停 | ドット click 後、明示再開まで自動ページ遷移 0。timer 同時発火では手動が勝つ。metadata・単独／同時切替でも停止を維持。再開は現在ページから新しい timer |
| D-AC10 | 自動一巡 | Q5-b 裁定後、最大件数・固定新着頻度・取消・再分割・受信ページ移動を継続する系列で、各継続有効 ID の初回実 paint 待ちと実 paint 間隔の最大値≤採用上限 L。新着による先頭復帰・timer 再開で未表示対象が飢餓になる件数 0。手動停止・背景時間は別報告し、受信演出時間は除外しない。自動 zoom・詳細をページ送りに付随させない |
...
| D-AC13 | 2 層・県操作 | 47 県・1,892 区域の coverage。県 click の dim・太線、市区町村 click の所属県選択、県外復帰、drag と click の分離。県選択中の別県 EEW／津波と単独→同時→単独で、危険色・不透明度が県 dim の影響を受けず、全緊急終了後に有効な手動状態へ復帰 |
| D-AC14 | EEW | 猶予秒・到達 countdown 0。津波なしの自動 camera 適用直後は全 active EventID 集合の震度 3 以上の全対応 geometry が bounds 内。欠損は明示。A 発表→B 発表→A 取消→B 続報→B 終了で残存 B の表示・緊急モードを維持。手動操作後は自動 bounds 保証から分離。通常／訓練の交差取消 0。full／summary は §8.1、通常合法最大は E13a |
| D-AC15 | 津波 | 地図主役、左大カード、大津波のみ点滅、全体図固定、「今すぐ避難！」と運用区分の明示。正規予報区の線・島・警報／注意報区分を検証し、県輪郭代用を合格にしない。full／summary は §8.1、通常合法最大は E13a。発令・引上げ・区域拡大と解除・降格の実表示は E01 |
| D-AC16 | 片側・同時発生 | 同時モードは両大カードの有効性で判定。EEW 継続中の大津波→警報→注意報→解除、注意報→EEW、最大件数・取消を系列化。両大カード有効中は EEW 宣言高・右残余の 48px 見出し・ドットを維持。片側では空きに応じ上位から昇格。進捗・手動停止・県 dim 保留を引き継ぎ、全対象へ到達。容量条件は §8.1・E13a |
...
| D-AC21 | 版付き詳細 | 正常取得、ページ切替、対象／他分野更新、取消、再起動、遅延旧応答、409、満杯、2 秒 timeout、worker 停止・client 切断を検証。異なる対象内容版・運用区分の合成 0、旧詳細の現在値表示 0。§7.10 の件数・byte・in-flight 上限内、main の full view 複製 0、停止未確認処理への重ね呼出し 0。詳細負荷中も E01 を満たす |
...
| D-AC24 | 実表示性能 | E01 の EEW・津波受信→実表示、E04 の snapshot 受信→paint、E25 の層切替、E26 の提示フレーム間隔を別々に検収する。最大値・標本数・条件・時計誤差・描画証拠を併記し、CPU 描画時間や rAF callback を実提示の代用にしない |
...
interface CorpusSequenceStep {
  ...
  evaluatedAt: number;
  inputSource: "ws" | "rest" | "replay" | "test"; operationExpected: Operation | "rejected" | null;
  expectedRef: string;
...
|---|---|---|---|
| ①-1 | 一電文で永続状態を最大 3 回 JSON 化 | §4.2 の保存単位、§5.1・§5.7 の変更単位 encode | §9.4 の無関係 U-W encode 0・変更検出 stringify 0。E10/E15 で無関係単位の巻込みと、同一単位の受理→通知完了／失効までの encode 回数・総 byte・worker 占有を分離計測する |
...
| ②-8 | VPWS50 history による肥大 | §4.5 の保持上限、§4.2 の U-W 分離 | 全国履歴 2・官署別 8 の境界と取消復元、無関係単位の encode 0。E10/E15 で履歴を含む同一単位の受理→通知完了／失効までの encode 回数・総 byte・worker 占有を計測する |
...
| ⑥-2 | owner と保存単位の不一致 | §4.2・§4.4 の整合性単位 | 一入力一保存単位、無関係 encode 0。単位内統合契約は F18。E10/E15 で intent 同居に残る同一単位の encode 回数・総 byte・worker 占有を計測し、整合性の費用として追跡する |
...
§7.7・§9.4・§10.6 の性能・資源判定は本節を共通定義とし、§9.4.1 の display 条件と併用する。

試験 manifest は入力 hash・順序・投入時刻、運用区分、初期 checkpoint、設定、時計、端末、OS、Node／Chrome、viewport／DPR、実行構成、client 数、personal consumer、測定器、標本数、期待値と根拠を含む。未記入のまま正式測定しない。

通常負荷 N は採用実入力窓の時刻間隔を維持した replay、ピーク負荷 P は固定倍率 replay と最大正常入力・同時最大状態の系列とする。窓・倍率・最大状態を結果を見る前に凍結する。queue の周期末比較には、更新・取消・期限処理が実際に生じる固定周期負荷 C を別に用意し、周期長・入力・時刻／revision の進め方・warm-up 終了点を固定する。重複拒否だけの反復で代用しない。

接続条件は backend 単独、display 1 client、最大 8 client、旧新並走、personal 有効を分ける。故障注入 F と意図的な容量超過は通常合法最大と分離する。

| ID | 試験条件・測定対象 | 判定式・報告 | 凍結担当・時期 |
|---|---|---|---|
| E01 | §7.5 の EEW・津波各母集団。詳細射影中も含む | 各 run の遅延上界 p99≤250ms、欠落 0。時計境界不確かは未確認。津波の発令・引上げ・区域拡大と解除・降格を混合しない | 測定＋統合担当。EEW は P2 A/B 前、津波は P3 契約前、製品は P4/P5 前 |
| E02 | N・P 中、独立 client から `/healthz` を毎秒要求。確立済み接続で要求開始→本文受信完了 | 暫定 p99≤100ms。transport 応答と worker 正常判定を分離し、stalled を正常と返さない | B11 担当、P2 性能測定前 |
| E03 | 最大正常 XML、worker 処理開始→decision・公開用射影確定。decode・parse・reduce・射影を含む | 暫定 p99≤1秒。queue 待ちは別計数し、parse だけの値に置換しない | B04・測定担当、P2 前 |
| E04 | 資材読込済み Chrome、意味 snapshot 受信→対象内容の実 paint | 暫定 p99≤250ms。上流待ちを含む E01、層切替の E25 と区別 | display 担当、P4 性能測定前 |
| E05 | N・P、Node プロセス RSS を毎秒。worker を含み Chrome は別集計 | 暫定 N 最大≤300MiB、P 最大≤400MiB | 測定担当、P2 前。製品構成は P5 前 |
| E06 | 保持上限を充足し、更新・取消・期限回収が各巡で発生する固定系列を 60 分。10 分窓を 6 窓 | RSS 窓中央値の回帰傾き≤暫定1MiB/時、最終中央値≤初窓＋暫定5MiB。FD 最終最大≤初窓最大。queue は E07。超過原因未分類のまま合格にしない | 測定担当、P2 長時間試験前 |
| E07 | N と固定周期負荷 C を各 60 分。queue の件数・byte・最古待機年齢、未完了年齢、周期末 backlog、最後の入力停止 | 全遷移で宣言上限内。通常入力の最大待機年齢は暫定5秒以下。入力停止から暫定10秒以内に入力 mailbox の pending／in-flight が0。C の warm-up 後は周期末 backlog の件数・byte が前周期末以下。3窓連続の年齢最大値増加は要調査シグナルであり、それだけでは不合格にしない | B05＋統合担当、P2 前 |
| E08 | 保存済み state、intent・batch・I/O なし、期限未到達。固定時計で無変化 tick 1,000回 | 全状態 clone・変更検出 stringify・checkpoint write・内容 snapshot 生成が各0。heartbeat 等は別計数 | runtime 担当、P2 契約前 |
| E09 | 正常・拒否・ignore 入力を別系列化 | 必要時の base64・展開は各1回以下。Body を読む XML は full parse 1回、事前拒否・ignore は0を許す。出力別再 parse 0 | B04 担当、P1 契約前 |
| E10 | U-W 保存済みから他単位だけ更新。U-W 期限未到達 | U-W 全量 encode 0。別理由で既に dirty な系列を混ぜない。同一単位の費用は E15 と分離 | checkpoint 担当、P2 前 |
| E11 | 正常／取消／no-op／最大入力 | 変更検出の全状態 stringify 0。手動全量コピー・JSON roundtrip も監査し、必要な変更枝構築・codec encode は別計数 | runtime 担当、P2 前 |
| E12 | 小型・大型・最大 XML。旧新を同一入力・初期状態・Node で別実行 | allocation profiler の割当量推定、GC 回数・停止時間、heap 前後を報告。RSS・heapDelta を割当総量と呼ばず、改善主張は同条件差分だけ | 測定担当、P2 前 |
| E13 | 全正常 corpus と同時最大状態。既知 GIS／REST 未対応は理由付き別群 | 意味上の恒常的 unavailable 0。必要な期限処理後も残るものを数える。配送 summary は別状態として報告 | 各単位担当、実装契約前 |
| E13a | 固定した通常合法最大 manifest。U-T の観測最大、U-E の複数 EventID、他分野との同時最大、運用区分混在を含む | U-E の区域別予想震度・U-T の予報区別警報区分の期待集合と配送値が一致し、欠落・誤値 0。summary・個別取得待ちを免除理由にしない。全 snapshot は1MiB以下。実現手段は §8.1 | B08・display＋統合担当、P4 容量契約前 |
| E14 | 同時 dirty、連続更新、遅い旧世代 ack。一単位だけ encode／書込み／照合が永続失敗し他単位は正常。停止未確認 I/O は別故障系列 | 正常単位の各 dirty 更新を含む世代 ack まで≤3秒、current 巻戻し0。終了済み失敗は1/2/4/8/10秒の再試行間隔を下回らず、正常単位を飢餓にしない。停止未確認時の書込み重複0、期限超過を明示 | checkpoint 担当、P2 B07 契約前 |
| E15 | 保存試行ごとに unit・generation・stage・byte。受理から通知完了／失効までの系列を対応付ける | checkpoint／tmp／ログ／採用 export への write 帰属、帰属不能0。retry は理由付き。同一単位の encode 回数・総 encode/write byte・worker 占有時間を報告し、byte/保存・受理・日も示す。改善量の根拠なし閾値を置かない | checkpoint・運用担当、P2／P5 前 |
| E16 | Pi 並走中、MemAvailable・disk を毎秒 | 暫定 MemAvailable≥2GiB、空きdisk≥10GiB。未達は新築 shadow 停止判断 | 運用＋統合担当、P5 開始前 |
| E17 | Pi swap counter・throttle 現在 bit を毎秒、10秒窓差分 | 暫定 swap-in/out 正の窓が3連続、または新規 throttle が30秒継続で停止判断。起動前の履歴 bit は別扱い | 運用担当、P5 開始前 |
| E18 | 旧築単独と旧新並走、同一入力・速度・設定の30分窓を各3回 | 旧系受信 callback 入口→同期受理終了の p99 を比較。`新窓−基準窓≥100ms` かつ `新窓≥1.2×基準窓` が2窓連続で停止判断。同一 ID 群1,000件以上。不足時の延長窓は試験前固定。非同等・標本不足は未確認 | 測定担当、P5 開始前 |
| E19 | Pi 72時間。N・P・障害系列、display・personal 条件を実行計画へ配置 | 全系列実施、上限内、未知の欠落・未分類障害0。重大変更後は影響する連続試験を再実施。稼働時間だけで合格にしない | 運用＋統合担当、P5 開始前 |
| E20 | 通常終了、保存不能、worker 停止、batch 残存、終了中の遅い ack | §5.9 の順序・世代・終了 code・時間上限が一致。保存未確認の code0 は0 | B02・checkpoint 担当、P2 前／全単位 P3 |
| E21 | adapter／consumer の成功・reject・timeout・abort 無視・遅い成功。通常 backlog・再試行・満杯・実行中へ津波緊急／EEW を投入 | §6.3・§12.7 の無効化・隔離・欠落、停止未確認処理への重ね呼出し0。固定優先順と満杯採否が一致。正常 adapter の正式緊急系列で各初回待ち≤暫定1秒。隔離・期限切れは成功に数えない | B09・B10＋統合担当、EEW P2／津波 P3 契約前 |
| E22 | 履歴復元中の live 更新・取消、期限横断、coverage 不足、運用区分交差 | 中間 current 公開0、新規履歴通知0、期限延長0、live 上書き0、区分間の取消・coverage 混入0。未充足理由を保持 | 復旧担当、各 P3 単位契約前 |
| E23 | ログ最大量・sink 失敗・再起動・秘密値例外 | level・保持上限を維持、再起動後読取可、秘密値露出0、sink 失敗の再帰増殖0 | B15・運用担当、P2／P5 前 |
| E24 | GIS の機械検査と目視検収 | code・coverage・geometry は数値結果、境界・島・細部は指定画面と原資料に対する担当判定。目視未実施を機械 Pass で代替しない | GIS＋検収担当、P4 検収前 |
| E25 | 資材読込済みの層切替。要求 handler 入口→要求した市区町村層の最初の実 paint | 暫定 p99≤250ms。固定 zoom・対象県・資材・viewport で各条件100 warm-up、1,000標本×3 run、各 run 合格。10秒以内に paint なしは欠落、不合格 | display・測定＋統合担当、P4 性能測定前 |
| E26 | 前景 Chrome・固定60Hz表示で連続 drag／wheel。実際に提示された地図フレーム間隔 | 暫定 p95≤16.7ms、p99≤33.3ms。CPU 描画時間でなく提示間隔を計数し、無提示時間を落とさない。条件・計数は下記。各 run 合格、証拠不足は未確認 | display・測定＋統合担当、P4 性能測定前 |

E02〜E04・E25 は各条件 1,000 標本以上・3 run、nearest-rank とする。E01 は §7.5、E26 は時間固定の以下の条件を用いる。標本不足は未確認とし、最大値条件を p99 へ置き換えない。

**E07 の計数**：enqueue・dequeue・転送・完了・取消の各遷移で件数・byte 上限を検査し、high-water と違反回数を保持する。待機年齢は enqueue から処理開始まで、未完了年齢は完了までと分ける。件数が変化しない滞留も既存の進捗監視で上限判定し、次の dequeue まで違反を隠さない。毎秒値は時系列報告に使う。C の周期末 backlog は pending と in-flight を含み、終了境界で測る。故障注入の停止未確認処理は正常排出合格に含めない。

通常待機 5 秒・停止後排出 10 秒は、既存の進捗監視・終了待機と対応させた**暫定値**だ。緊急入力の待ちを 5 秒まで認める値ではない。B05 担当が予備測定し、統合担当が P2 前に固定する。3窓増加を検出したら年齢・backlog・排出結果を報告し、実際の累積滞留と瞬間的な burst 差を分ける。

**E26 の操作条件**：

- 資材・初期 camera・zoom・viewport・DPR・motion・Chrome・端末を固定し、表示 refresh rate は 60Hz とする。別 refresh rate の結果を混ぜない。
- drag と wheel を別系列とし、それぞれ操作開始前 10 秒を warm-up、連続操作 60 秒を 3 run 行う。
- 入力は毎秒 60 回の固定 trace とする。drag の経路・CSS px/秒、wheel の deltaMode・delta 値・方向転換・zoom 範囲を具体値と trace hash で P4 測定前に凍結する。clamp に当たり続けて画面変化がなくなる列は使わない。
- compositor／描画 trace から実際の地図提示時刻を取得する。連続する提示時刻の差に、測定開始から初回提示、最終提示から測定終了の無提示区間も含める。途中停止を削除したり、実行された描画関数の短い CPU 時間だけを集計したりしない。
- 1 run ごとに有効な提示間隔 1,000 標本以上、p50／p95／p99／max、無提示最大時間、入力数、提示数を報告する。提示なし、10 秒以上の無提示、trace 欠落で停止区間を評価不能な場合は合格にしない。
- 16.7／33.3ms は 60Hz の約1／2フレームに対応する暫定開始目標であり、実証済み値ではない。SVG／Canvas／WebGL は同じ trace・資材・条件で比較する。

E06 の回帰傾きは窓中央時刻と窓中央値の最小二乗直線とする。1MiB/時・5MiB は allocator の揺れを許す経験的暫定値であり、保持上限・queue 上限の構造検査と併用する。

E12 は allocation 観測を必須とするが、根拠のない全入力共通 byte 閾値を置かない。全状態コピーは E11、実資源は E05/E06 で判定する。

E15 の同一単位全量 encode は intent 同居に残る整合性の費用として扱う。E01 または保存 3 秒の予算を侵す実測が出た場合だけ、期限内の短い合流・不変枝再利用を比較する。別 outbox DB・差分 journal を先回りして追加しない。

その他の暫定値の根拠は、100ms・1秒・300/400MiB が既存開始目標、2GiB・10GiB が旧系を残す運用余裕、20%かつ100ms が微小差での誤判定を避けるためである。正式試験前に担当が予備測定し統合担当が凍結する。結果後の緩和で旧結果を合格化せず、新しい試験版にする。

72時間には通常・ピーク・停止排出・再接続・保存失敗と復帰・終了と復元・通知失敗・consumer 隔離を含める。別環境の故障試験を Pi 実施と偽らない。構造ゼロ条件は counter と静的な経路確認を併用し、実装と同じ分岐を写した oracle を作らない。

**契約補完と実装検証済みは別**であり、§9.8 の実施結果がない行を Pass に変更しない。
...
公開 query は `getStatus`、`getUnitView(unit)`、§7.10 の型を使う `getDisplayDetail(request)` を開始範囲とする。`getUnitView` は公開済みの有界な分野 DTO と単位 revision を返し、snapshot が summary ならその省略状態を保持する。main に full view を複製する抜け道にしない。詳細は同じ worker 射影・上限・対象内容版・運用区分・timeout を使い、内部 state・codec・任意 filesystem を公開しない。command は §11 の採用操作の型付き union に限り、不正引数・未知 command・任意関数を受付前に拒否する。
...
P0は、未決事項を消して見せるphaseではない。P1作業契約 `P1-PARSER-BOUNDARY-001`（`reconstruction/contracts/`）は 2026-09-15 に起草固定し、check-manifest／check-sequences／check-contract の 3 検査が PASS した（発注OIDは統合担当が設定）。P0 の完了資材 ①〜④ の残りは §9.3 の REST 取得計画だけで、未決の裁定（Q5 の P4 前）とは独立に扱う。
...
- （2026-09-12、v4）ChatGPT Pro 独立レビュー IR01〜IR14 を契約へ反映。単位別保存再試行、運用区分の全経路伝播、EEW→津波緊急→その他の通知順、通常合法最大での危険区域配送欠落0、原因別 A/B 判定、worker 詳細ページ射影、対象別の意味鮮度解除を明文化した。
- ご主人裁定 IR05=A により津波の受信→実表示も p99 250ms を開始目標とし、発令・引上げ・区域拡大と解除・降格を別系列化した。IR11=A により受信ページ移動で巡回進捗を巻き戻さず、継続有効 ID の実 paint 最大待ちを受入へ追加した。Q5-b の推奨60秒は保留を維持する。
- 緊急時の県 dim 保留、津波階級に依存しない同時配置、全 active EventID に対する bounds・終了条件を追加。詳細取得 timeout は5秒から2秒の共通契約へ変更し、地図層切替 E25・提示フレーム間隔 E26 を定義した。
- E07 の三窓連続増加は要調査シグナルへ変更し、最大待機年齢・排出期限・周期末 backlog を合否条件とした。IR15 は台帳対応表へ E10/E15 の残余費用計測を追記した。本改訂は契約補完であり、実装・試験・P0成果物の完成を意味しない。
