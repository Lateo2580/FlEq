...
| 通知文言 | overall なら現共通 formatter は `最大震度不明（未入電）`。地域だけの未入電は通知本文へ列挙しない | overall なら `最大震度5弱以上未入電` | §7.5.4 D の自然文テンプレートへ移行し、`未入電` を必ず明示する |
| CLI／ticker | CLI は `不明` 固定、非 CLI 面は `不明（未入電）` | 現 HEAD は `5弱以上未入電` | §7.5.4 B／C の `5弱以上（未入電）` 一体表示へ統一する。raw semantic の保持は実装済み |
...
基盤 semantic、safety rank、色、badge は裁定対象外である。次の A〜D は文字列とカード内の情報配置について確定した。未採択案を実装またはテスト期待値へ残さない。

- **A: plain 未入電地域の表示文言** — CLI 以外のカード、地図 tooltip／ARIA、地域 group は `不明（未入電）` と表示する。CLI 詳細は §3.7 に従い `不明` を固定する。
- **B: `5弱以上未入電` の履歴行** — `5弱以上（未入電）` の独立 group／chip とし、`≥` badge を付ける。exact 震度5弱として扱わない。
- **C: `5弱以上未入電` のカード** — `5弱以上（未入電）` を qualifier 一体の主 label とし、`≥` badge を付ける。`未入電` を副 label へ分離しない。tooltip／ARIA は Condition／Description を保持する。
- **D: 通知文言** — 自然文で `未入電` を必ず明示する。`5弱以上未入電` は、たとえば「最大震度は5弱以上とみられます（未入電）」の形とする。テンプレートの助詞・句読点は既存 notifier の実装慣行に合わせて確定してよいが、qualifier を落とさない。
...
5. **地図・カード・履歴**: all-unknown の sentinel wire、unknown host 選択・5分TTL・tier・既存地図置換、既知 emergency→unknown 保存、unknown／lower-bound の色・badge、裁定 B／C、tooltip／ARIA を固定する。frontend は `unknownHost` を protocol から受け、`deriveQuakeMapHostEvent()` で known host を優先した上で、未期限の unknown host だけを選択する。largeQuake がある間、または known host が有効な間は unknown host を画面選択しない（`src/engine/display/intensity-groups.ts`、`src/engine/display/project-event.ts`、`src/engine/display/protocol.ts`、`src/engine/display/state-store.ts`、`display/frontend/src/lib/protocol.ts`、`display/frontend/src/lib/derive.ts`、`display/frontend/src/lib/quake-map-colors.ts`、`display/frontend/src/components/LatestQuakeCard.svelte`、`display/frontend/src/components/RecentQuakes.svelte`、`test/engine/display/project-event.test.ts`、`test/engine/display/quake-map-state.test.ts`、`test/engine/display/standby-state-store.test.ts`、`display/frontend/src/lib/__tests__/derive.test.ts`、`display/frontend/src/components/__tests__/quake-map.test.ts`、`display/frontend/src/components/__tests__/latest-quake-card.test.ts`、`display/frontend/src/components/__tests__/recent-quakes.test.ts`、`src/engine/messages/daily-quake-persistence.ts`（実装時裁定 2026-08-26: 単位5 を単独投入したときの新旧 qualifier label 互換に必要なため対象へ追加。新旧 persisted row の round-trip テストを完了条件に含める）、`test/engine/telegram-foundation/phase4a-contract.test.ts`（同裁定: 表示変更に伴う機械的期待値更新のみ）。xhigh NO-GO 補修（実装時裁定 2026-08-26）: unknownHost・map contribution・large-quake reference・revision の lifecycle 跨ぎ（§7.5.3(5)(6)）を成立させるため、`src/engine/display/runtime.ts`・`src/engine/display/quake-display-store.ts`（新規）・`src/engine/display/quake-display-persistence.ts`（新規・additive `quake-display-v1.json`・EventID 単位 salvage）・`src/engine/monitor/monitor.ts`・`src/engine/monitor/display-sink.ts`・`src/engine/monitor/shutdown.ts`・`test/engine/display/quake-display-persistence.test.ts`（新規）を対象へ追加し、display off 中受信→TTL 内 on・既知 emergency→restart→unknown 続報・known→unknown 直後 restart の 3 production-shaped シナリオを完了条件に含める。
