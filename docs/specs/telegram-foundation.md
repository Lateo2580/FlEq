...
第3縦切り（裁定待ち・ご主人判断）: browser active card の新規導入、card type／kind union、layout solver、component、standalone VPBS50 card の製品判断、および card reconcile は本契約の完了後に別契約で扱う。本縦切りの ticker receipt、identity、TTL、metric を card state へ流用してはならず、対応 fixture と layout 契約を先に確定する。
