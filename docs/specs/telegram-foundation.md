...
6. 6B後半（本骨組みの対象外）: 実 pair fixture ごとに parser extractor、confirmed counterpart／severity rule、pair revision・取消、明示 display reconcile/remove と card／frontend、protocol sync、実到着順 integration test を一つの縦切りで追加する。対象ファイルと TTL／card identity は fixture 調査後に別契約で確定し、本契約から推測しない。
