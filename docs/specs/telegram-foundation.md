...
- source record の `receivedAtMs`、60秒 deadline、11分 expiry は最初の受理時に固定し、timeout release、訂正、遅着で延長しない。同一 source subject の受理済み訂正／strictly newer revision が Holdback 中に来た場合は payload と revision を置換するが、最初の deadline／expiry は保つ。transport／semantic duplicate、stale、invalid meta／revision は相関器へ入れない。correlation cache と raw revision family registry は、同じ11分値を使う場合も定数、entry、prune、capacity、責務を共有しない。
...
- timer callback は entry token／generation を照合し、置換前 payload、dispose 後 callback、timeout と counterpart 到着の競合から二重 emit しない。`dispose()` は source／counterpart／expired-source tombstone の全 timer と Map を破棄し、shutdown 中に新規表示・通知を合成しない。restart 後は空 cache から始め、旧 process の counterpart 観測を抑止根拠にしない。
...
- **確定裁定（2026-08-11 ご主人裁定）:** 骨組みの表示 surface は専用 CLI＋通常 ticker までを実装し、correlator は遅着一致時の typed `reconcileLateCounterpart` action と source identity を unit test まで固定する。active card の追加と `DisplayIngestSink` の原子的 reconcile/remove API は6B後半へ送る。同一 `groupKey` の上書きだけでは hub の `recent` に source が残り、canonical domain 固有 identity／TTL と衝突し得るため採用しない。generic legacy card と frontend authoritative ticker sync の先行実装も、実 counterpart の card identity と TTL を fixture なしで固定するため採用しない。
...
- 6B後半へ持ち越す: 実 pair での suppression と到着順同値、timeout 後の canonical active 表示、card／ticker からの source 原子的除去、実 code の high／non-high 通知、pair をまたぐ訂正／取消と source 復帰のproduction有効化、`legacyLateCounterpartReconciled` の成功後加算。従って既存完了条件のうち「片系だけの fail-open」「restart」「他 ignore／回帰」と訂正／取消の純粋状態遷移は骨組み対象、「対応電文確認時だけ抑止」「両順序の最終 active 表示一致」「high 通知」「実 pair rule による訂正／取消」は6B後半対象とする。
...
6. 6B後半（本骨組みの対象外）: 実 pair fixture ごとに、(a) parser extractor、confirmed counterpart／severity rule、実到着順 integration test を含む rule/severity slice と、(b) 明示 display reconcile/remove、card／frontend、protocol sync、`legacyLateCounterpartReconciled` を含む display reconcile slice の二段へ分割する。前者の完了を Phase 6B 全体完了とは呼ばない。各 slice の対象ファイル、TTL、card identity は対応 fixture と前 slice の確定契約から定める。
...
- `LegacyCounterpartRule` に `eligibleInfoTypes?: readonly ("発表" | "訂正" | "取消")[]` 相当の additive admission filter を追加する。filter は source／counterpart とも correlation cache へ入れる前に評価し、VPOA50 rule は `eligibleInfoTypes:["発表"]` とする。非対象 counterpart は既存 domain の通常 emit だけを行い cache を変更しない。非対象 source は共通 gate による受理後、cache、Holdback、candidate、late reconcile を一切変更せず即時 fail-open 表示へ進める。さらに VPOA50 の `InfoType=取消` は、admission filter と独立に Kind.Code／Condition／Status の値を問わず severity を `unknown` へ固定し、high 昇格経路を構造的に遮断する。従って未観測の訂正／取消が、既存 cache の候補置換・解除・復帰を偶然起こさず、取消の即時 emit が通知適格にならない。
...
- VPOA50 が一意 match した場合だけ source を抑止し、VPBS50 自体の既存 briefing parser、severity、通知、card／ticker、revision gate は変更しない。VPBS50 が VPOA50 と無関係な場合も既存経路をそのまま通る。VXWW50／VPNO50 の60秒後 fail-open、三 source type 以外の ignore、他 domain の相関・表示・通知・stats・永続化は不変とする。

訂正・取消の保守側確定裁定（実 fixture 取得後に再裁定）:

- **保守側で確定（2026-08-23）:** 実 6 pair はすべて `InfoType=発表` であり、訂正／取消 pair は未観測である。`eligibleInfoTypes:["発表"]` により、VPOA50／VPBS50 の訂正・取消は type 間 suppression、candidate 置換、source 復帰、late reconcile の根拠にしない。VPOA50 訂正は source-local の共通 revision gate を通して fail-open 更新し、Kind.Code `1` と発表状態を確定できる場合だけ §11.8／U5 の `訂正`＋`対応電文未確認` 通知を一回許可する。VPOA50 取消は、`markCancelled` の EventID subject 受理が既存 active state を要求せず、legacy DTO の `groupKey` も `null` であるため、実取消 fixture を得るまで**表示解除を行わない非通知 fail-open**とする。severity は常に `unknown` へ固定し、受理は共通 revision gate の既存挙動に完全に委譲する。本縦切りでは取消に追加の診断・metric を設けず、既出表示は TTL 満了へ委ねる。VPBS50 側の訂正／取消は既存 briefing 契約を変えない。VPOA50 の取消 remove policy と pair をまたぐ訂正／取消は、両側実 fixture を得た次縦切りで再裁定する。

late reconcile の確定裁定:

- **裁定済み（2026-08-23、A 採択）:** 本縦切りは Holdback 内 suppression と timeout 後の unmatched high 通知までとする。timeout 後に VPBS50 が到着した場合、既存 typed `reconcileLateCounterpart` action により VPBS50 は通常 emit するが、表示済み VPOA50 の active card／ticker は除去せず、`legacyLateCounterpartReconciled` も加算しない。現在の `DisplayIngestSink` は `ingest()` だけで legacy DTO の `groupKey` は `null` のため、source 除去と canonical card 差し替えを原子的に保証できない。ここで同一 `groupKey` 上書きを導入すると hub `recent`、VPBS50 固有 identity、TTL が競合するため、fixture／rule／通知の有効化と表示 protocol 変更を分離する。この既知の残存差分は §11.5 の最終状態をまだ満たさず、Phase 6B 全体完了とは数えない。
