...
薄いエントリポイントとすることで、テスト時に `engine/cli/cli.ts` 以下を個別にインポート・テストしやすくしている。
...
| `./engine/cli/cli` | `buildProgram()` — Commander プログラム定義 |
...
`PromptStatusProvider` は `TsunamiStateHolder`, `VolcanoStateHolder` が実装し、REPL が `role` を theme に解決して津波警報・火山警戒レベルを表示する。
...
| `TelegramListResponse` | 電文リスト API レスポンス (`items`, `nextToken`, `error`) |
| `TelegramListItem` | 電文リストの各アイテム (`serial`, `id`, `classification`, `head`, `xmlReport`, `body?`) |

`TelegramListItem` は `WsDataMessage` と類似の構造を持つが、REST API 経由で取得するため `serial` フィールドが追加されている。`rest-client.ts` の `listTelegrams()` が返す。
...
| `ParsedVolcanoAlertInfo` | VFVO50, VFSVii | パース済み噴火警報・予報 (kind: `"alert"`) |
| `ParsedVolcanoEruptionInfo` | VFVO52, VFVO56 | パース済み噴火観測報・噴火速報 (kind: `"eruption"`) |
| `ParsedVolcanoAshfallInfo` | VFVO53/54/55 | パース済み降灰予報 (kind: `"ashfall"`) |
| `ParsedVolcanoTextInfo` | VZVO40, VFVO51 | パース済み火山テキスト情報 (kind: `"text"`) |
| `ParsedVolcanoPlumeInfo` | VFVO60 | パース済み推定噴煙流向報 (kind: `"plume"`) |
...
- `PromptStatusProvider` は REPL と状態管理の結合を疎にするためのインターフェース。状態管理側は chalk や formatter を参照せず raw text / roleを返し、色付けは UI 境界で行う。
...
外部依存なし。以下のモジュールから参照される:
- `ui/formatter.ts` — 震度色の解決
- `engine/notification/level-helpers.ts` — フレームレベル判定時の震度比較
- `engine/messages/summary-tracker.ts` — 要約統計での震度ランク算出
- `engine/presentation/rank-maps.ts` — PresentationEvent の maxIntRank 算出
