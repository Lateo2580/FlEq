# spec: 竜巻注意情報カード内ページ送り（v1.0, 2026-08-23）

> v0.3 → v1.0: ご主人裁定（2026-08-23）により D1=A、D2=A、D3=B、D4=C を確定。採用案を規範へ一本化した。
...
- 計算順は固定する。(1) snapshot から候補・reset key・契約高を決める、(2) side / center で必要な合成 probe を enqueue、(3) 全 required probe が確定するまで旧 confirmed registration を維持し provisional range を表示、(4) 確定 partition 群を atomically publish、(5) coordinator を register する。probe 結果が registration / solver 高を変えて新 probe を誘発する循環は許さない。
...
- 確定して 1 地域 range も fail したとき、(i) aggregate fallback `竜巻注意情報（対象 N 地域）` を forced probe、(ii) aggregate fit なら `infeasible=aggregate`、(iii) aggregate も fail なら契約高内の固定最終 clip / ellipsis を出し `infeasible=clip` とする。最終 clip は「全地域を読めた」と扱わない。empty / cancellation は既存 state の責務であり fallback を起動しない。
...
## §4 裁定済み（2026-08-23 ご主人）

### D1: weather 本文と tornado rider のページ座標

- **採用: 案 A（独立 pager + weather host、2026-08-23）**。tornado は独立 `PageableKey` / identity を持ち、rotation では weather appearance に依存する。共通 paging/probe key、§3.2 の固定契約高、host weather が 1 page でも dependent が進むこと、全 live 組成の safety envelope、§3.4 の publish 順による settle 非振動を満たす。
- 不採用経緯: 案 B は複合 pager、案 C は `max(Pw, Pt)` の zipped coordinate だった。
- **設計上のフォールバック条件**: 上の採用条件、とくに safety envelope の証明を満たせない場合は、独立 pager を実装せず案 B / C を再評価する。これは実装レビューの blocking 条件である。

### D2: 1 地域も fit しない場合

- **採用: 案 A（2026-08-23）**。aggregate fallback → aggregate probe → 最終 clip / ellipsis の三段防衛とし、`aggregate|clip` を区別する。
- 不採用経緯: 案 B は先頭 1 地域 fallback → probe → 最終 clip / ellipsis だった。

### D3: marker の DOM・位置・ラベル

- **採用: 案 B（2026-08-23）**。weather は既存 footer、tornado は rider 行末に `対象地域 2/4` を inline 表示する。marker 同士、weather body、rider text の rectangle overlap は 0 とする。
- 不採用経緯: 案 A は card 共通 footer の二段 / 二セグメント、案 C は複合 pager 用の単一 marker だった。

### D4: `isSighted` と reset

- **採用: 案 C（2026-08-23）**。`false→true` の危険側変化だけ reset、`true→false` は active page を維持する。単純な resetKey の値切替では実現しないため、前回の `isSighted` を比較して増分時だけ escalation generation を進める、または coordinator registration に当該遷移だけを示す `forceReset` を渡す機構を実装単位で選ぶ。
- 不採用経緯: 案 A は上下とも非 reset、案 B は上下とも reset だった。
...
- [ ] 集合・順序変更は reset。`isSighted` は `false→true` だけ reset、`true→false` は active page を維持する。同一 resetKey / escalation generation の再分割で active identity が消えた場合は successor-after-removal へ遷移し、先頭 reset を起こさない。
...
- [ ] 1 page は marker なし、複数 page は rider 行末の inline `対象地域 k/P` を出す。weather footer と marker、weather body、rider text の overlap は全て 0。
- [ ] 1 地域 fail → aggregate fallback probe → fallback 自身が fail なら最終 clip の多段防衛を通し、`data-tornado-page-infeasible` が `aggregate` / `clip` を正しく区別する。
