...
`WebSocketManager` は接続のライフサイクル全体を管理し、障害発生時には指数バックオフで自動再接続を行う。古いソケットのイベント遅延到着に対するガード（`this.ws !== socket` チェック）により、接続の切り替え時の競合状態を防止している。
...
2. `ws` パッケージでサブプロトコル `dmdata.v2` を指定して接続
3. `open` イベントで `reconnectAttempt` をリセットし、ハートビートを開始、`onConnected` を発火
4. `message` イベントで `handleMessage()` を呼ぶ
...
4. `onDisconnected` イベント発火
5. `scheduleReconnect()` で再接続をスケジュール
...
- **インポート元**: `ws` (WebSocket クライアント), `../types` (`AppConfig`, `WsDataMessage`, `WsStartMessage`, `WsPingMessage`), `./rest-client` (`prepareAndStartSocket`), `../logger`
- **接続先**: `engine/monitor.ts` から `WebSocketManager` がインスタンス化される
...
- **接続先**: `engine/message-router.ts` から各パース関数が呼ばれる
