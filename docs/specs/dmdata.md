...
| 対策 | 概要 | 状態 |
|------|------|------|
| **JSON ファイル出力機能（`EventFileWriter` / `--event-log`）を公開版から除去** | 電文を構造化 JSON として永続化する機能は public repo から除去する。個人開発者本人の利用は private overlay 運用に移行する | 未着手（`src/engine/events/event-file-writer.ts` 等が存続中） |
