...
緊急地震速報電文（VXSE43/44/45）をパースする。震源情報、予測震度地域、仮定震源要素の検出、最終報判定（`NextAdvisory`）、isWarning の XML ベース判定を行う。
...
#### isWarning の XML ベース判定

EEW 電文の `isWarning` フラグは以下の 4 条件の OR で判定する（上から優先）:
...
4. `msg.classification === "eew.warning"` — WebSocket 配信ラベル（フォールバック）

XML ベース判定（1-3）と classification（4）が食い違った場合は `log.warn` で観測ログを出力する。
