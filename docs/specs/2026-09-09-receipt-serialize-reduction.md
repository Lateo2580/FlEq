...
#### 段階 3-B の Pi 実測（窓 4、2026-09-09 19:58〜20:20、稼働 SHA `da14d1b`、committed 11 通）

main `a63d642`（CI 緑）→ personal `da14d1b` → Pi。`FLEQ_PERF_RECEIPT` なしの本番配線で観測。

| 受入 D | 結果 | 判定 |
|---|---|---|
| serB 消失（hit 時） | 8/8 で消失 | 達成 |
| ヒット率 | 88.9%（8/9） | 参考値（受入条件に閾値なし） |
| VPWS50 大型 total 中央値 | 2,134 → **1,502ms（−30%）** | 達成（1,900ms 以下） |
| 小型 VPWW55/56 total 中央値 | 1,192 → 1,107ms（−7%） | **未達**（1,000ms 以下に届かず。見込み 960ms も外れ） |
| heapDeltaMB / 永続ファイル | 49MB・v2 +0.6% で横ばい | 参考値 |
| D1（v2 / v1 バイト数） | v2 **3.06MB**（窓 1 の頃 1.4MB） | 採取済み。3 窓連続未達だった C7 を解消 |

`[perf-*-lost]` 0 件。

**読みどころ。** 大型電文は見込み（1,856ms）を上回って改善し、`serB` を丸ごと消す設計が
狙いどおり効いた。一方で小型電文は見込み未達で、`serD`（307 → 370ms、+21%）が伸びている。
原因は D1 で判明した永続状態 v2 の肥大（1.4MB → 3.06MB、1 日で 2 倍以上）で、`serD` は
状態サイズに比例するため base pair キャッシュの対象外（`serD` は毎回走る）である。
**小型が伸びなかったのは 3-B の欠陥ではなく、状態肥大という別の原因が `serD` 側で
...
#### 段階 3-E＋3-D の Pi 実測（窓 5、2026-09-10 05:03〜06:52、稼働 SHA `40d4f58`、committed 20 通）

`FLEQ_PERF_RECEIPT=1` で採取、`[perf-*-lost]` 0 件。committed の内訳は VPWS50 11・
VPWW61 4・VPWW56 2・VPWP50 2・VXKO53 1。**今朝の VPWS50 は 70KB 級**（窓 3・4 は
110KB 級）なので、大型の `total` は窓 4 と直接比較しない。

| 区分（中央値） | 窓 4 | 窓 5 | 判定 |
|---|---|---|---|
| 小型 `total`（n=7） | 1,107ms | **704ms（−36%）** | §9.9 の「小型 1,000ms 以下」も達成 |
| 小型 `serD` | 370 | **193（−48%）** | |
| 小型 `serIn` / `serEnc` | 未採取 | 163 / 64 | 窓 1 の `serIn` 589（§8.5）・窓 3 の 533〜546（§9.8）比で −70% |
| 小型 `save` / `sweepPre` | — | 32 / 0.6（precheck） | |
| VPWS50 70KB `total`（n=11） | （110KB: 1,502） | 1,197 | `parse` 520（7.4ms/KB）、`disp` 178、`serD` 188 |
| base pair ヒット率 | 88.9% | 73%（11/15、`serB` 残存 4 行） | 低下、要因未調査 |
| v2 サイズ（5 分 sampler、VPWS50 11 通 committed 後） | 3.06MB | **最大 1.82MB**（1.37〜1.82MB） | **受入 3-E 達成**（2.1MB 以下） |

**帰属の注意。** `serD` の減少幅は状態サイズ比（1.7 / 3.06 = 0.56 → 370 × 0.56 ≈ 206 ≒
実測 193）でほぼ説明できる。**この窓では 3-E（履歴 8 → 2）の効果が支配的で、3-D
（lossless assert の省略）単独の寄与は A/B が無く分離できない。** 3-D 単独の値は Mac 実測
（`serIn` −52%、§9.12）を持つ。受入 3-D「`serIn` が窓 4 比 −40% 以上」は、窓 4 に
`serIn` 行が無いため窓 1 / 窓 3 の `serIn` を基準に読んで数値上は満たすが、
**「達成（帰属は 3-E と混在）」**と記録する。

Pi は 06:52 に通常運用へ復帰した（環境変数なし、healthz 200）。
...
17. **base pair キャッシュを実装した**（実装記録は §9.10）。開発機 2MB 状態で
    ...
    **起草時の「serializer は domains と envelope の純関数」は旧 dep 経路では
    ...
    Pi 窓 4（受入 B・D1）は採取済み（§9.10）

### 2026-09-09 段階 3-B Pi 窓 4 採取（`da14d1b`）

18. **受入 D を確認した**（実測は §9.10）。VPWS50 大型 total 2,134 →
    1,502ms（−30%、見込み 1,856ms を上回って改善）、serB は hit 時 8/8 で消失、
    ヒット率 88.9%。**小型 VPWW55/56 は 1,192 → 1,107ms（−7%）で見込み
    960ms 未達**。D1（v2 / v1 バイト数）を初めて採り、永続状態 v2 が
    1.4MB → 3.06MB に肥大していることが判明した。`serD` が 307 → 370ms
    （+21%）と伸びており、小型電文が見込み未達だった原因は状態肥大側にあると読む
19. **段階 3-D の着手判断は本 spec の外へ持ち越した。** ご主人裁定待ち
    （2026-09-09 夜ハンドオフ §要裁定）。判断材料は状態肥大の切り分けが
    先という向きで揃いつつある

### 2026-09-10 段階 3-E＋3-D Pi 窓 5 採取（`40d4f58`）

20. **受入 3-E を確認した**（実測は §9.12 末尾）。v2 は VPWS50 11 通 committed 後も
    最大 1.82MB（窓 4 の 3.06MB から −40%）。小型 `total` 1,107 → 704ms（−36%）、
    `serD` 370 → 193（−48%）で、§9.9 の「小型 1,000ms 以下」も初めて達成した
21. **受入 3-D は「達成（帰属は 3-E と混在）」。** `serD` の減少は状態サイズ比で
    説明でき、A/B が無いので 3-D 単独の寄与は Pi では分離できない。Mac 実測（−52%）を
    3-D 単独の根拠として残す。base pair ヒット率 73% への低下（窓 4 は 88.9%）は
    要因未調査
