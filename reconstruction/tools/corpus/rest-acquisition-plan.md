...
|---|---|---|---|---|---|---|
| O01 | 4, 11 | VXSE43/45 同 revision 訂正 | 2024-04-17 | 外 | **D（Archive）のみ**（EEW 保存開始 2022-07-20 以降）。EEW は Telegram List に出ないので経路 A の別事例張り替えも不可 | VXSE43: **取得失敗**（`eew.warning` の archive 一覧が 402 `Not contract.`）→ synthetic。VXSE45: **候補あり**。`archive.data` 権限は通った（2024-04-18 JST 分 72 通を取得、VXSE45 12 通・全部発表。ただし錨の 4/17 23:14 JST は **2024-04-17 分**に入る: 312 通・145,436 byte、未取得・要承認） |
...
| O01 | 37–39 | VFVO55 火山 506 訂正・取消・重複 | 2021-05-14 | 外 | D（Archive、保存開始後）。別事例は A | **候補あり**。2021-05-15 JST 分（106 通、VFVO53 96・VFVO52 10、全部発表）を取得したが錨の 5/14 12:51 JST は **2021-05-14 分**（112 通・411,977 byte、未取得・要承認）。直近 VFVO55 24 件は全て発表 |
...
| O03 | 14 | VPWW55 福井 code00 | 2026-08-30 | 内 | A（`datetime` 指定を試す） | **本文確認済み・code00 なし**: 福井 11:40 JST 報（14,429 byte）は「特別警報から危険警報」への降格報で、code00（解除）は含まない。同日の後続報（15:03・20:04 JST）が code00 候補（本文確認待ち 2 件、要承認） |
