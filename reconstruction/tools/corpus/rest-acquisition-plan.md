...
| Archive List v2（`GET /v2/archive`、`archive.list` 権限、**契約中の配信区分のみ**）は配信区分ごとの日別 `.tar.gz` の一覧。応答は `id／classification／date／dataCount／fileSize／url`、`datetime` で日付範囲指定。本体は Archive Data v1（`GET https://data.api.dmdata.jp/v1/archive/:id`、**別権限 `archive.data`**）で、解凍後の `telegrams.json` を参照して個別電文を辿る。同じ id への短期間の反復要求は禁止。**保存開始**: 地震津波・火山・気象警報・定時報は **2020-11-18 12 時**、EEW（予報・警報）は **2022-07-20 15 時**。**2026-09-16 実測**: `date` は JST 日（ファイルは前日 15:00Z〜当日 14:59Z）、`datetime=A~B` は **B の日付のファイル**を返す（錨の日 D を取るには `datetime=D-1~D`）。各電文は `.xml` と `.json` の両方で入り、`telegrams.json` は `classification／filename／format／head／id／receivedTime／xmlReport` の配列。`archive.list`・`archive.data` とも FlEq 契約で通った（eew.forecast・telegram.volcano。**eew.warning は 402 `Not contract.`**） | dmdata docs `archive.list/`・`v1/archive.data/`、`~/dev/fleq-corpus-p0/index.json` |
...
|---|---|---|---|---|---|---|
| O01 | 4, 11 | VXSE43/45 同 revision 訂正 | 2024-04-17 | 外 | **D（Archive）のみ**（EEW 保存開始 2022-07-20 以降）。EEW は Telegram List に出ないので経路 A の別事例張り替えも不可 | VXSE43: **取得失敗**（`eew.warning` の archive 一覧が 402 `Not contract.`）→ synthetic。VXSE45: **探索範囲内に見つからず**。2024-04-17 JST 分（312 通・145,436 byte、追加承認 A で取得）に EventID 20240417231454 の VXSE44/45 が各 33 報（serial 1〜33）あるが全部「発表」で同 revision 訂正は無い → synthetic。副産物: 同 EventID の EEW 33 報連鎖は原本として O01/O09 の発表系列に使える |
| O01 | 29–32 | VFVO50 火山 306 続報・訂正・取消・重複 | 2020-05-22 | 外 | **Archive 保存開始（2020-11-18）より前**。通常の探索対象から外す。別事例は A（噴火警報の取消は稀事象） | 別事例: VFVO50 直近 18 件（2026-04-28〜09-12）は全て発表・訂正なし → **探索範囲内に見つからず**。ただし **VFVO52 に同 EventID `20260807015800_506` の 発表 01:58→訂正 02:07（serial 1 同士）の実連鎖あり。本文取得済み（2,630／2,658 byte、差分は Control/DateTime・Head/ReportDateTime・Head/InfoType（発表→訂正）・PlumeDirection の値と description 属性（東→西）・Body 末尾 `<Text>流向を訂正</Text>` の 5 点（2026-09-16 レビューで訂正）。EventID・Serial 1・TargetDateTime は同一で、同 Serial・新 ReportDateTime の訂正。envelope parse OK）**。噴火 slice の錨として張り替え候補 |
| O01 | 37–39 | VFVO55 火山 506 訂正・取消・重複 | 2021-05-14 | 外 | D（Archive、保存開始後）。別事例は A | **探索範囲内に見つからず**。2021-05-14 JST 分（112 通・411,977 byte、追加承認 A で取得）は VFVO51 10・VFVO52 6・VFVO53 96 で **VFVO55 は 0 通**（錨 12:51 JST は span 内）。既存 fixture の由来は archive では裏付けられない。直近 VFVO55 24 件も全て発表 → synthetic |
...
| O02 | 3 | VPWP50 稚内 合法 empty | 2026-06-05 | 内 | A（`datetime` 指定を試す） | **探索範囲内に見つからず**: `datetime` で 6/5 に到達（100 件）。稚内の 18:01 JST 報は無く、23:00 JST 報を本文取得（261,597 byte）したが TimeSeriesInfo 3・Item 30 で empty ではない。合法 empty は synthetic |
| O02 | 21–22 | VPTA50 TC2606 全ゼロ後続報・gate-only | 2026-06-02 | 内 | A | cursor で 3 ページ目（2026-04-11〜06-22）に到達。TC2606 は 34 報で**最終報は serial 87（6/2 15:55、既存 `76_01_03` と同じ）。serial 88・89 は存在しない → 探索範囲内に見つからず**（全ゼロ後続報・gate-only newer は synthetic）。副産物: 既存 `76_01_02`（serial 41）・`76_01_03`（serial 87）の配信 item が一覧にあり、本文 2 件で原本確認へ昇格できる（別段階） |
...
| O03 | 14 | VPWW55 福井 code00 | 2026-08-30 | 内 | A（`datetime` 指定を試す） | **候補あり・本文確認済み**: 福井 8/30 の実連鎖 11:40（特別警報→危険警報、14,429 byte）→ 15:03（危険警報／警報→注意報、14,594 byte）→ **20:04（`<Code>00</Code>` 解除 ×16、11,562 byte）**。O03 の code00 錨として張り替え候補（同日 10 報あり、残り 7 報は未取得） |
