...
/**
 * 気象電文 (VPWS50 / VPWW55-61 / VPNO50) の confirmed な受理で昇格状態を更新する。
...
export function applyWeatherPromotionOnIngest(
  ...
  if (!isVpws50StateHeadType(event.type) && event.type !== "VPWW56" && event.type !== "VPNO50") return false;
  if (event.weatherConfidence === "unsafe") return false;
  if (((isVpws50StateHeadType(event.type) && event.type !== "VPWS50") || event.type === "VPWW56" || event.type === "VPNO50")
