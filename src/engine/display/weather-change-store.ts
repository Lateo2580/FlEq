...
/** VPWS50/VPWW55-61 の受理済み差分だけを保持する、非永続・単一レコードの表示ストア。 */
export class WeatherChangeDisplayStore {
  ...
  apply(event: PresentationEvent, nowMs: number): boolean {
    if (!isVpws50StateHeadType(event.type)) return false;
    ...
    this.record = {
      expiresAtMs,
      dto: {
        // VPWW55-61 は全国 view に重ねる部分 stream だが、wire 上の権威 source は統合 state の vpws50。
        source: "vpws50",
