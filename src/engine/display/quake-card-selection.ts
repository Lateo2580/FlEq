...
export function quakeCardRank(candidate: QuakeCardCandidate): number {
  return candidate.maxIntSemantic?.safetyRank ?? candidate.maxIntRank ?? 0;
}
...
export function shouldReplaceQuakeHost(
  ...
  return quakeCardRank(candidate) >= quakeCardRank(current);
}
